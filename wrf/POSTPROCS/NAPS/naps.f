...
c     --- end of GETBLAST
      ...
      END
C
      SUBROUTINE GETSITE(JSITE,NOGUN,IERR)
c     --- Retrieve lat,lons on specified site
C         XB,YB     - UTM's of the blast site.
C         XLAT,XLONG- lat. and long. of the blast site.
      IMPLICIT NONE
...
c
c     print *,'enter GETSITE: JSITE=',JSITE
      IERR=0
      NOGUN = .TRUE.
      IF(JSITE .EQ. 1) THEN
      ...
      ELSE
        IERR=-1
        print *,'ERROR IN SITE SPECIFICATION'
        RETURN
      END IF 
      xlong=-xlong
      LATB=XLAT
...
cmm5  --- MM5 change.  Check for blank records
  ...
  222 format(I2)
      do kread=1,5
        read(MT3,199,IOSTAT=IOS) record
        if(IOS.NE.0) then
          IERR=-1
          return
        endif
...
cmm5  --- isolate sounding lat,lon
      ...
      record=FILLABEL
      CALL shftlft(record)
cyliu NL=index(record,'i,j=')
cyliu if(NL.LE.0) go to 287
      NL=index(record,'Init')
      if(NL.LE.0) go to 287
cyliu
      read(record(49:54), '(F6.2)', IOSTAT = IOS) LATS
      read(record(58:65), '(F8.3)', IOSTAT = IOS) LONS
c         snd_label=record(NL:)
c         FILLABEL=record(1:NL-2)
      go to 2887
 ...
 2887 continue
      write(KT,*) 'in MET: LATS,LONS=',LATS,LONS
      write(6 ,*) 'in MET: LATS,LONS=',LATS,LONS
  287 continue
      do kread=1,5
        read(MT3,199) record
        CALL shftlft(record)
...
c       WRITE(* ,3000) II,HEIGHT,TC,RO,WS,WD,EAST,NORTH,press
  499   CONTINUE
c       print *,'kount,nlevels=',kount,nlevels
...
crds  SUBROUTINE USER(IOPT,IDBFLG)
      SUBROUTINE USER(JOPT,IDBFLG,IERR)
C-----------------------------------------------------------------------
...
c
c     write(kt,*) 'on entry to BLAST: NPPSPK=',NPPSPK
c     write(kt,*) 'ELEV='
c     DO I=1,73
...
c     print *,'ISUR=',ISUR
      IF(ISUR.LE.-1) THEN
c        IF(IRANGE .NE. 0) THEN
c          DEGANG = ANGLE(IRANGE)* 180./PI
...
c           print *,'all raytraces invalid: THETM=',THETM
           GO TO 8
        END IF
        GO TO 2000
      ENDIF
...
C  Display the met angle so all print outs are consistant.
 ...
 600  FORMAT(////35X,'PEAK SOUND INTENSITIES IN DB ALONG AZIMUTH (MET)',
     1 ' OF',F6.2,' DEG',//)
       WRITE(MT8,611)
 611  FORMAT(1H ,3X,' RAY NO.  ELEV ',5X,'RANGE (M)',4X,'RAY APOGEE(M)',
     12X,'ARRIVAL',3X,'FREE AIR',3X,'CHARGE',3X,'ONE RAY',3X,'MANY RAYS'
     2,2X,'TOTAL',4X,'TOTAL',4X,'NO OF',/,13X,'ANGLE',34X,'TIME(S)',4X,
     3'(DB)',5X,'WT.(DB)',3X,'FOCUS(DB) FOCUS(DB) 1LB.(DB)',3X,'(DB)',
     44X,'RAYS',3X,'TYPE',/)
      DO 400 I=1,IRANGE
         JSKIP=MAX(ISK(I),1)
         DO 400 J=1,JSKIP
...
c##################################################
c     if(SIP.eq.0.) then
c       write(6 ,*) 'in NOISE: SIP=',SIP
c       WRITE(6 ,*) 'I,IS,SND,AW,A,DIRDB,DECIBL(I,IS)=',
...
c-----------------------------------------------------------------------
     ...
     &                WV(70),SV(70),p_mb(70),NP
c-----------------------------------------------------------------------
      INTEGER IPLOT, ISCALE
      REAL    XPLOT, YPLOT, APLOT, XTEMP, YTEMP 
      COMMON /TRCINP/ IPLOT, XPLOT(751), YPLOT(751), APLOT(751),
     &                        XTEMP(751), YTEMP(751), ISCALE
c-----------------------------------------------------------------------
      integer ISI,I,N,NN,NBRPTS
      real    input_thmin,DELSI,VMAXU,VMAXL,SVdry,VN,VSOUND,SOUNDC,
     1        SOUNDU,SOUNDV,THETCH
      real    SK,C,U,W,COSANG
	real    PI,REARTH
...
c       write(kt,*)'N,ZB,ALT,SVdry,SV,V=',N,ZB,ALT(N),SVdry,SV(N),V(N)
  ...
  65  CONTINUE
cmm5  --- ensure no trapping in the first layer
c     if(V(2).GE.V(1)) then
c       V(1)=V(2)+0.5
c     endif
      if((VMAXL.LE.0.) .AND. (ZB.LT.ALT(1))) VMAXL=V(1)
...
cmm5  --- try angles at least up tp 1 deg
      input_thmin=1.   ! deg
c     input_thmin=2.   ! deg
c     input_thmin=0.5  ! deg
      THETM=MAX(THETM,input_thmin) 
...
c       print *,'IP,ISI,ELEV,RP=',IP,ISI,ELEV(IP,ISI),RP
...
C Raytrace went beyond max range without ground impact.
 ...
 36     CONTINUE
 35     CONTINUE
c       print *,'after 35: ICOUNT,ILIMIT,NPPSPK,IREF=',
...
C  Correct vertical profile due to current ground elevation.
     ...
     &       3.* ABS(ZGRND-ALT(1))))
         ENDIF
      END IF
c     print *,'after 30: ICOUNT,ALTC,ALTN=',ICOUNT,ALTC,ALTN
c
...
c  INB, for speed of sound value.
      ...
      Z2=ALTC
c     print *,'Z1,Z2,IN,INBINT=',z1,z2,inb,inbint
C
...
c     --- plot speed of sound vs ht in m/s
   ...
   15 continue
      dc = cmax-cmin
      print *, 'psi,cmax,cmin,kp=',DEGX,cmax,cmin,kp
      do k=1,kp
        xp=(V(k)-cmin)*10./dc
        xcoord(k)=xp
        yp=ALTN(k)
...
c
      ...
      mjrx=ns
      mnrx=1
      print *,'in plotray: calling set: UL,UR,UB,UT=',UL,UR,UB,UT
      call set(FL,FR,FB,FT,UL,UR,UB,UT,LL)
      dU = UT-UB
      call labmod('(I2)','(I5)',0,0,2,2,20,20,0)
      call periml(mjrx,mnrx,mjrz,mnrz)
...
c       --- open the file
cmm5    CALL GETSITE(JSITE,NOGUN,IERR)
cmm5    ALT(1) = ELEV(1,1)
cmm5    IEL =-1
cmm5    call plotmet2
cmm5  END IF
...
c       --- DBPLOTOPT=5: filled contour lines on WVS filled background
        ...
        elseif(DBPLOTOPT.EQ.4) then
          print *,'calling PLOTDB for option 4'
          cntfill=.true.
          chfill=.false.
          CALL PLOTDB3(cntfill,chfill)
...
c----------------------------------------------------------------------
c  This routine will place a circle and number at the location of 
c  various microphones surrounding APG.
c  Logical input parameter colorfill is used to plot microphone locations
c  only if .FALSE., otherwise will plot only those microphones having
c  data, color coded according to the ICOLRS scheme.
...
c-----------------------------------------------------------------------
      ...
      integer i,numm,icolor,ios,j,jj,jrec,lc,nmikes,nrec,nreadings
      real    reading  ! dB
      real    xcord,ycord,xl,yl,blastlat,blastlon
      integer mikeid(maxm)
      real    mikedb(maxm)
      logical plotdata
      CHARACTER*80 record
...
c     enddo
c
c     --- Read in microphone data from the blast if available
      jj=0
      if(colorfill) then
        OPEN( UNIT = 14, FILE =DATADIR(1:ncdir)//'mikedata.dat',
     *      STATUS = 'OLD', FORM = 'FORMATTED', iostat=ios)
        if(ios.eq.0) then
c         --- read the data
          plotdata=.TRUE.
          READ(14,1111,END=200) record  ! header
          READ(14,*,END=200) blastlat,blastlon
          do j = 1,maxm
            READ(14,*,END=200) mikeid(j),mikedb(j)
            jj=jj+1
          enddo
        else
	    plotdata=.FALSE.
...
c-----------------------------------------------------------------------
      ...
      character*10 namem(maxm)
      real    degs,mins,secs,hchar
      integer i,numm,icolor,ios,j,jj,jrec,lc,nmikes,nrec,nreadings,ier
      real    reading  ! dB
      real    xcord,ycord,xl,yl,blastlat,blastlon,r,theta,psi
      real    uvb(3),uvm(3)
      integer mikeid(maxm)
      real    mikedb(maxm)
      ...
      integer ls
      logical plotdata
      CHARACTER*80 record,mikedsn
      REAL*8  REARTH,DEGREES_PER_RADIAN
      PARAMETER (REARTH = 6370.999D0)  ! km
      parameter (DEGREES_PER_RADIAN = 57.29577951308232D0 )
c
c     print *,'entering mikes0'
      call getset(xx1,xX2,yY1,yY2,xUL,xUR,yUB,yUT,ls)
c     print *,'in mikes0: after getset: X1,X2,Y1,Y2,UL,UR,UB,UT=',
...
c     --- Read in microphone data from the blast if available
      jj=0
      if(colorfill) then
        nrec=MAX(INDEX(met_dsn,'.')-1,1)
        mikedsn=DATADIR(1:ncdir)//met_dsn(1:nrec)//'mikes.dat'
        OPEN(UNIT=14,FILE=mikedsn,
     *      STATUS = 'OLD', FORM = 'FORMATTED', iostat=ios)
        if(ios.eq.0) then
c         --- read the data
          plotdata=.TRUE.
          ...
          READ(14,*,END=200) blastlat,blastlon
c         print *,record 
c         print *,blastlat,blastlon
          do j = 1,maxm
            READ(14,*,END=200) mikeid(j),mikedb(j)
c           print *, j,mikeid(j),mikedb(j)
            jj=jj+1
          enddo
        else
          plotdata=.FALSE.
  ...
  200 continue
      nreadings=jj
c     print *,'nreadings=',nreadings
c     print *,'plotdata=',plotdata
c
c     --- Plot the microphone locations
      if(white_backgrnd) then
        lc=0
      ...
      call defuvp(blastlat, blastlon, uvb)
      do i=1,nmikes
c       --- Find the x,y position relative to blast
        call defuvp(latm(i), lonm(i), uvm)
        call abetuv(uvb,uvm,theta)  ! ang dist
        r = REARTH*theta         ! m
        call comphd(uvb, uvm, psi, ier) ! clockwise from north
        theta=90.-psi   ! cc from east
        ...
        if(plotdata) then
          do jj=1,nreadings
            if(nmike(i).eq.mikeid(jj)) then
              reading=mikedb(jj)
c             print *,'reading=',reading
c             --- find the appropriate color
              do j=1,NCONTS-1
                if((reading.ge.contrs(j)).and.(reading.lt.contrs(j+1)))
     &          then
                  icolor=j
c                 print *,'xcord,ycord,icolor=',xcord,ycord,icolor
c                 --- fill a circle with the appropriate color
                  CALL NGWSYM('N',8,XCORD,YCORD,0.015,ICLORS(icolor),0)
c                 --- overlay an open circle with the background color
                  CALL NGWSYM('N',0,XCORD,YCORD,0.015,lc,0)
c                 --- number the location
                  record(1:2)='  '
                  write(record,230) nmike(i)
                  CALL shftlft(record)
                  ...
                  yl=ycord+0.5*hchar*(UT-UB) ! 1/2 Char ht up
                  if(xcord.GT.0.) then
                    xl=xcord+0.5*hchar*(UR-UL) ! 1/2 Char ht to right of circle
                    call plchhq(xl,yl,record(1:nrec),hchar,0.,-1.)
                  else
                    xl=xcord-0.5*hchar*(UR-UL) ! 1/2 Char ht to left of circle
                    call plchhq(xl,yl,record(1:nrec),hchar,0.,+1.)
                  endif
c                 print *,'i,nmike(i),x,y=',i,record(1:nrec),xcord,ycord
                  go to 250
...
c     ---- build a rectangular grid
      ...
      ixc=nxg/2+1
      jyc=nyg/2+1
c     print *,'dxg,dyg,xlg,ylg=',dxg,dyg,xlg,ylg
c
...
c
c     --- Draw in the microphone data if available
      call mikes0(.true.)   ! true=colorfill
c
...
c-----------------------------------------------------------------------
     ...
     1  FILOUT,met_dsn,ncdir,nodir,nmdir
      LOGICAL DBFLG
      integer i, j, ix, jy, i1, i2, ls, ier, ipt1, ipt2
...
c     --- read in the bay, state and county boundaries
     ...
     1   access='sequential',form='formatted',status='old',iostat=ios)
      if(ios.ne.0) then
...
c     xleb=0.96
c     xreb=0.995
      xleb=MIN(X2+(1.-X2)/2.+.01,0.96)
      xreb=xleb+.035
c     print *,'X2,xleb,xreb=',X2,xleb,xreb
      ybeb=0.5
      yteb=0.75
      call gsfais(1)  ! solid fill
