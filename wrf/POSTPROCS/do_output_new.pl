...
require "$POSTPROCS_DIR/do_sites+bias.pl";
...
if ( -e  "/data/GMODJOBS/$JOB_ID/postprocinput.pl" ) {
  require "/data/GMODJOBS/$JOB_ID/postprocinput.pl";
   ...
   $DO_MDV_3 = 1 if(! $DO_MDV_3);
   $DO_MDV_4 = 1 if(! $DO_MDV_4);
   $DO_GEM_1 = 0 if(! $DO_GEM_1);
   $DO_GEM_2 = 0 if(! $DO_GEM_2);
   $DO_GEM_3 = 0 if(! $DO_GEM_3);
   $DO_GEM_4 = 0 if(! $DO_GEM_4);
...
##            $MM5_FILE = "${RUNDIR}/${THIS_CYCLE}/${MM5_DIR}/MMOUT_DOMAIN$d${SUFFIX}";
         ...
         system("rm 2004*MMOUTPUT_DOMAIN$d.$RANGE");
...
# Make some directories on 4dwx.org, so the web page points to a real directory
#
$USER_DIR = "/www/htdocs/images/$lrange/rtfdda_gmod";
$JOB_LOC = "/www/htdocs/images/$lrange/rtfdda_gmod";
...
if ( $DO_RIP ) {
   ...
   print "PLOT_DEST_DIR = $PLOT_DEST_DIR\n";
}
#
...
#          Run the scipuf converter to generate medoc files
           ...
           &do_scipuf( $RUNDIR . "/postprocs/" . $FILED3, $IS_FINAL, 1, $SCIPUF_DEST_LOC3);
        }
        if ( $DO_MEDOC_4 ) {
#          Run the scipuf converter to generate medoc files
           $SCIPUF_DIR = $SCIPUF_DIR4;
           $SCIPUF_ARCHIVE_DIR = $SCIPUF_ARCHIVE_DIR4;
           &do_scipuf( $RUNDIR . "/postprocs/" . $FILED4, $IS_FINAL, 1, $SCIPUF_DEST_LOC4);
        }
        if ( $DO_NAPS_2 ) {
#          Copy the file to the web server for NAPS-interactive
           &do_naps( $RUNDIR . "/postprocs/" . $FILED2, $NAPS_DEST_DIR2, $THIS_CYCLE, $NAPS_CYCLE_DIR);
        }
        if ( $DO_NAPS_3 ) {
#          Copy the file to the web server for NAPS-interactive
           &do_naps( $RUNDIR . "/postprocs/" . $FILED3, $NAPS_DEST_DIR3, $THIS_CYCLE, $NAPS_CYCLE_DIR);
        }
        if ( $DO_MDV_1 ) {
#          Copy the file to the web server for NAPS-interactive
           &do_mdv( $RUNDIR . "/postprocs/" . $FILED1, $MDV_DEST_DIR1, $MDV_DEST_HOST, $MDV_DEST_LOC1);
        }
        if ( $DO_MDV_2 ) {
#          Copy the file to the web server for NAPS-interactive
           &do_mdv( $RUNDIR . "/postprocs/" . $FILED2, $MDV_DEST_DIR2, $MDV_DEST_HOST, $MDV_DEST_LOC2);
        }
        if ( $DO_MDV_3 ) {
#          Copy the file to the web server for NAPS-interactive
           &do_mdv( $RUNDIR . "/postprocs/" . $FILED3, $MDV_DEST_DIR3, $MDV_DEST_HOST, $MDV_DEST_LOC3);
        }
        if ( $DO_MDV_4 ) {
#          Copy the file to the web server for NAPS-interactive
           &do_mdv( $RUNDIR . "/postprocs/" . $FILED4, $MDV_DEST_DIR4, $MDV_DEST_HOST, $MDV_DEST_LOC4);
        }
        if ( $DO_GEM_1 ) {
#          Copy the file to the web server for NAPS-interactive
           &do_gempak( $RUNDIR . "/postprocs/" . $FILED1, $GEM_DEST_DIR, $valid_time, "1");
        }
        if ( $DO_GEM_2 ) {
#          Copy the file to the web server for NAPS-interactive
           &do_gempak( $RUNDIR . "/postprocs/" . $FILED2, $GEM_DEST_DIR, $valid_time, "2");
        }
        if ( $DO_GEM_3 ) {
#          Copy the file to the web server for NAPS-interactive
           &do_gempak( $RUNDIR . "/postprocs/" . $FILED3, $GEM_DEST_DIR, $valid_time, "3");
        }
        if ( $DO_GEM_4 ) {
#          Copy the file to the web server for NAPS-interactive
           &do_gempak( $RUNDIR . "/postprocs/" . $FILED4, $GEM_DEST_DIR, $valid_time, "4");
        }
#        if ( $DO_STEREO ) {
...
system("scrub 1 $RUNDIR/postprocs");
system("scrub 1 $WORK_DIR");
