...
sub do_plots_gcat
{
my ($hourly_file,$ndom, $cycle, $valid_time, $dest, $dest_small, $DEST_SERVER) = @_;
...
if ( $range == "GRM" && $do_tiles == 1 ) {
   ...
   foreach $tile (1..6) {
     ...
     system("cat /data/GMODJOBS/$JOB_ID/postprocs/namelists/Mdomain2.$season.$range | sed s/plttime/$xt/g > domain$tile.1");
     ...
     print ("conv2gif( domain$tile.cgm, tile=$tile, $valid_time\n");
...
#    Now do the observation plots --- using a custom version of RIP
     ...
     system("cat /data/GMODJOBS/$JOB_ID/postprocs/namelists/Mdomain2.obs.$range | sed s/plttime/$xt/g > domain$tile.1");
     system("cat domain$tile.1 | sed s/rangename/$range/g > domain$tile.2");
     system("cat domain$tile.2 | sed s/cycle/$cycle/g > domain$tile.in");

     system("$RIP_OBS -f ../ripscr_obs/Domain_$tile domain$tile  > $PLOTS_DIR/rip_obs$tile.log 2>&1");
     print ("conv2gif_obs( domain${tile}.cgm, tile=$tile, $valid_time\n");
...
#  If this domain doesn't run as long as D1...
   ...
   print("cat /data/GMODJOBS/$JOB_ID/postprocs/namelists/Mdomain$d.$season.$range > domain$d.1\n");
   system("cat /data/GMODJOBS/$JOB_ID/postprocs/namelists/Mdomain$d.$season.$range | sed s/plttime/$xt/g > domain$d.1");
   ...
   if ( -e  "/data/GMODJOBS/$JOB_ID/sloc$d.in" ) {
      ...
      system("cat domain$d.3 /data/GMODJOBS/$JOB_ID/postprocs/namelists/Mdomain$d.sloc.$range > domain$d.in");
   }
...
## For EPG
   if ( $d == 2 && $DO_EPG == 1 ) {
   system("cat /data/GMODJOBS/$JOB_ID/postprocs/namelists/Mdomain4.$season.$range | sed s/plttime/$xt/g > domain4.1");
   ...
   if ( -e  "/data/GMODJOBS/$JOB_ID/sloc4.in" ) {
      ...
      system("cat domain4.3 /data/GMODJOBS/$JOB_ID/postprocs/namelists/Mdomain4.sloc.$range > domain4.in");
   }
...
## End for EPG
     ...
     print ("conv2gif( domain$d.cgm, domain=$d, $valid_time\n");
     &conv2gif( "domain$d.cgm", $d, $valid_time);
## For EPG
   if ( $d == 2 && $DO_EPG == 1 ) {
     print ("conv2gif( domain4.cgm, 4, $valid_time\n");
...
#    Now do the observation plots --- using a custom version of RIP
     ...
     system("cat /data/GMODJOBS/$JOB_ID/postprocs/namelists/Mdomain$d.obs.$range | sed s/plttime/$xt/g > domain$d.1");
     system("cat domain$d.1 | sed s/rangename/$range/g > domain$d.2");
     ...
     print ("conv2gif_obs( domain${d}.cgm, domain=$d, $valid_time\n");
     &conv2gif_obs( "domain$d.cgm", $d, $valid_time);

## For EPG
   if ( $d == 2 && $DO_EPG == 1 ) {
    system("cat /data/GMODJOBS/$JOB_ID/postprocs/namelists/Mdomain4.obs.$range | sed s/plttime/$xt/g > domain4.1");
     system("cat domain4.1 | sed s/rangename/$range/g > domain4.2");
     ...
     print ("conv2gif_obs( domain4.cgm, 4, $valid_time\n");
...
#LPC     system("scp -pr gifs_ugui/$valid_time/d$d\* $dest/$valid_time" );
     ...
     chdir gifs_ugui;
     system("rsync -e 'ssh -i /data/GMODJOBS/$JOB_ID/rtfdda.key' -avzC $valid_time $dest");
#LPC     if ( $d == 2 ) {
...
#LPC     system("scp -pr gifs_ugui/$valid_time/d$d\* 4dwx\@atec-server:/EPG/rtfdda/gifs/$valid_time" );
         system("rsync -e 'ssh -i /data/GMODJOBS/$JOB_ID/rtfdda.key' -avzC $valid_time 4dwx\@atec-content:/d1/www/htdocs/images/epg/rtfdda/gifs");
##        system("scp -pr $valid_time 4dwx\@atec-content:/d1/www/htdocs/images/epg/rtfdda/gifs");
#LPC     if ( $d == 2 ) {
...
##       system( "ssh 4dwx\@atec-server $scandir_command" );
       system( "ssh $DEST_SERVER $scandir_command" );
     }
   }
