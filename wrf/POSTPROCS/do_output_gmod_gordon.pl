...
if (! defined(@GRIB_INTRVL)) {  # in mins, best if defined in postprocinput.pl
   @GRIB_INTRVL = (60, 60, 60, 60);
}

if (! defined(@RIP_INTRVL)) {  # in mins, best if defined in postprocinput.pl
...
#          Copy the file to distrib area
           if ( $IS_WRF && $lead_time <= $NETCDF_LENGTH[0] ) {
...
#          Run the RIP script to generate graphics
           if ( $IS_WRF ) {
              ...
              $mn = $2;
              $minutes = $mn + $hh*60;
              if ($minutes%$RIP_INTRVL[1] == 0 &&
                  $lead_time <= $RIP_LENGTH[1]) {
                 if ($PROC_OBS_1) {
                    $PROC_OBS_2 = 0;
                 } else {
                    $PROC_OBS_2 = 1;
...
#          Run the RIP script to generate graphics
           if ( $IS_WRF ) {
              ...
              $mn = $2;
              $minutes = $mn + $hh*60;
              if ($minutes%$RIP_INTRVL[2] == 0 &&
                  $lead_time <= $RIP_LENGTH[2]) {
...
#          Copy the file to the mdv server for MDV conversion
        ...
        if ( $DO_GRIB_1 ) {
           if ( $IS_WRF ) {
             ...
             $mn = $2;
             $minutes = $mn + $hh*60;
             if ( ($minutes%$GRIB_INTRVL[0] == 0) &&  $lead_time <= $GRIB_LENGTH[0]) {
        ...
        if ( $DO_GRIB_2 ) {
           if ( $IS_WRF ) {
             ...
             $mn = $2;
             $minutes = $mn + $hh*60;
             if ( ($minutes%$GRIB_INTRVL[1] == 0) && $lead_time <= $GRIB_LENGTH[1]) {
               $dt = `date`;
        ...
        if ( $DO_GRIB_3 ) {
           if ( $IS_WRF ) {
             ...
             $mn = $2;
             $minutes = $mn + $hh*60;
             if ( ($minutes%$GRIB_INTRVL[2] == 0) && $lead_time <= $GRIB_LENGTH[2] ) {
               $dt = `date`;
        ...
        if ( $DO_GRIB_4 ) {
           if ( $IS_WRF ) {
             ...
             $mn = $2;
             $minutes = $mn + $hh*60;
             if ( ($minutes%$GRIB_INTRVL[3] == 0) && $lead_time <= $GRIB_LENGTH[3] ) {
               $dt = `date`;
