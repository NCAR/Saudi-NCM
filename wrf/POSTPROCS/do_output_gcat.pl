...
# Which post processors to run
   ...
   $DO_STEREO = 0 ;
   $DO_TILES = 0 ;
   $DEST_SERVER = "4dwx\@atec-server";
#  MDV not used in GCAT
   $MDV_DEST_HOST = "4dwx\@atec-ingest";
   $MDV_DEST_ROOT = "/raid/mm5/raw";
#  STEREO SERVER desactivated for GCAT, see line 325 
...
##            $MM5_FILE = "${RUNDIR}/${THIS_CYCLE}/${MM5_DIR}/MMOUT_DOMAIN$d${SUFFIX}";
         ...
         $OUTPUT_FILE = "MMOUT." . $d;
         system("cp $HOURLY_FILE $RUNDIR/postprocs");
         if ( -e $HOURLY_FILE_M1 ) {
           system("cat $HOURLY_FILE_M1 $HOURLY_FILE > $OUTPUT_FILE");
         } else {
           system("mv $HOURLY_FILE $OUTPUT_FILE");
         }
         system("rm 19*MMOUTPUT_DOMAIN$d.$RANGE 20*MMOUTPUT_DOMAIN$d.$RANGE");
...
require "$POSTPROCS_DIR/do_scipuf_gcat.pl";
require "$POSTPROCS_DIR/do_scipuf_prf_gcat.pl";
require "$POSTPROCS_DIR/do_sites_gcat.pl";
require "$POSTPROCS_DIR/do_plots+obs_gcat.pl";
require "$POSTPROCS_DIR/do_grid_plots_gcat.pl";
...
require "$POSTPROCS_DIR/do_stereo_vis_gcat.pl";
...
#$JOB_LOC = "/migs/vandenb/$USER/$JOB_ID";
...
#
# Directory for medoc/prf files
#
if ( $DO_PRF_2 ) {
   $SCIPUF_PRF_DIR2 = "$RUNDIR/prf2";
   $SCIPUF_PRF_ARCHIVE_DIR2 = "$RUNDIR/prf2_archive";
   $SCIPUF_PRF_DEST_LOC  = "$JOB_LOC/scipuff_prf";
   $SCIPUF_PRF_DEST_LOC1 = "$JOB_LOC/scipuff_prf1";
   $SCIPUF_PRF_DEST_LOC2 = "$JOB_LOC/scipuff_prf2";
   $SCIPUF_PRF_DEST_LOC3 = "$JOB_LOC/scipuff_prf3";
   $SCIPUF_PRF_DEST_LOC4 = "$JOB_LOC/scipuff_prf4";
   $SCIPUF_PRF_DEST_LOC5 = "$JOB_LOC/scipuff_prf5";
   $SCIPUF_PRF_DEST_LOC6 = "$JOB_LOC/scipuff_prf6";
   $SCIPUF_PRF_DEST_DIR  = "$DEST_SERVER:$SCIPUF_PRF_DEST_LOC/";
   $SCIPUF_PRF_DEST_DIR1 = "$DEST_SERVER:$SCIPUF_PRF_DEST_LOC1/";
   $SCIPUF_PRF_DEST_DIR2 = "$DEST_SERVER:$SCIPUF_PRF_DEST_LOC2/";
   $SCIPUF_PRF_DEST_DIR3 = "$DEST_SERVER:$SCIPUF_PRF_DEST_LOC3/";
   $SCIPUF_PRF_DEST_DIR4 = "$DEST_SERVER:$SCIPUF_PRF_DEST_LOC4/";
   $SCIPUF_PRF_DEST_DIR5 = "$DEST_SERVER:$SCIPUF_PRF_DEST_LOC5/";
   $SCIPUF_PRF_DEST_DIR6 = "$DEST_SERVER:$SCIPUF_PRF_DEST_LOC6/";
}
if ( $DO_PRF_3 ) {
   $SCIPUF_PRF_DIR3 = "$RUNDIR/prf3";
   $SCIPUF_PRF_ARCHIVE_DIR3 = "$RUNDIR/prf3_archive";
   $SCIPUF_PRF_DEST_LOC3 = "$JOB_LOC/prf";
   $SCIPUF_PRF_DEST_DIR3 = "$DEST_SERVER:$SCIPUF_PRF_DEST_LOC3/";
}
if ( $DO_PRF_4 ) {
   $SCIPUF_PRF_DIR4 = "$RUNDIR/prf4";
   $SCIPUF_PRF_ARCHIVE_DIR4 = "$RUNDIR/prf4_archive";
   $SCIPUF_PRF_DEST_LOC4 = "$JOB_LOC/prf";
   $SCIPUF_PRF_DEST_DIR4 = "$DEST_SERVER:$SCIPUF_PRF_DEST_LOC4/";
}
#
...
if ( $DO_STEREO ) {
# Addition for GCAT
   $DEST_SERVER_STEREO = "$DEST_SERVER";
   $JOB_LOC_STEREO = "$JOB_LOC/vis5d";
# End of addition for GCAT
...
#          Run the RIP script to generate graphics
           print ("do_plots_gcat $WORK_DIR $FILED2 $NDOMAINS $THIS_CYCLE $valid_time $PLOT_DEST_DIR $PLOT_SMALL_DIR $DEST_SERVER\n");
           &do_plots_gcat( $WORK_DIR . "/" . $FILED2, $NDOMAINS, $THIS_CYCLE, $valid_time, $PLOT_DEST_DIR, $PLOT_SMALL_DIR, $DEST_SERVER);
        }

        if ( $DO_SITES ) {
#          Run the tabular site forecast script
           &do_sites_gcat( $WORK_DIR . "/" . $FILED2, $NDOMAINS, $CYCLE_TAG, $SITES_DEST_DIR);
        }
#        if ( $DO_MEDOC_2 ) {
...
#          Run the scipuf converter to generate medoc files
	   ...
	   print ("do_scipuf_gcat $WORK_DIR/$FILED3, $IS_FINAL, 1, $SCIPUF_DEST_LOC3\n");
           &do_scipuf_gcat( $WORK_DIR . "/" . $FILED3, $IS_FINAL, 1, $SCIPUF_DEST_LOC3, $JOB_ID);
        }
        if ( $DO_MEDOC_4 ) {
#          Run the scipuf converter to generate medoc files
           $SCIPUF_DIR = $SCIPUF_DIR4;
           $SCIPUF_ARCHIVE_DIR = $SCIPUF_ARCHIVE_DIR4;
	   print ("do_scipuf_gcat $WORK_DIR/$FILED4, $IS_FINAL, 1, $SCIPUF_DEST_LOC4\n");
           &do_scipuf_gcat( $WORK_DIR . "/" . $FILED4, $IS_FINAL, 1, $SCIPUF_DEST_LOC4, $JOB_ID);
        }
        if ( $DO_PRF_3 ) {
#          Run the scipuf converter to generate medoc files
           $SCIPUF_DIR = $SCIPUF_PRF_DIR3;
           $SCIPUF_ARCHIVE_DIR = $SCIPUF_PRF_ARCHIVE_DIR3;
	   print ("do_scipuf_prf_gcat $WORK_DIR/$FILED3, $IS_FINAL, 1, $SCIPUF_PRF_DEST_LOC3\n");
           &do_scipuf_prf_gcat( $WORK_DIR . "/" . $FILED3, $IS_FINAL, 1, $SCIPUF_PRF_DEST_LOC3, $JOB_ID);
        }
        if ( $DO_PRF_4 ) {
#          Run the scipuf converter to generate medoc files
           $SCIPUF_DIR = $SCIPUF_PRF_DIR4;
           $SCIPUF_ARCHIVE_DIR = $SCIPUF_PRF_ARCHIVE_DIR4;
	   print ("do_scipuf_prf_gcat $WORK_DIR/$FILED4, $IS_FINAL, 1, $SCIPUF_PRF_DEST_LOC4\n");
           &do_scipuf_prf_gcat( $WORK_DIR . "/" . $FILED4, $IS_FINAL, 1, $SCIPUF_PRF_DEST_LOC4, $JOB_ID);
        }
...
#FVDBsystem("rm -rf $WORK_DIR/plots");
#FVDBsystem("rm -f $WORK_DIR/*MMOUT*");
