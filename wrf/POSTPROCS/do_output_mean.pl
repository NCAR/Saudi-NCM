...
#
# This script controls the processing of one output-time-step of ensemble-mean RTFDDA data
# It is called from rtfdda_postmean.pl
...
#
my (@parts) = split "/", $RUNDIR;
$NODE_MEM = $parts[$#parts];
# FVDB
$NODE_MEM = "MEAN";
require "$HOMEDIR/data/GMODJOBS/$JOB_ID/flexinput.pl";
if ( -e  "$HOMEDIR/data/GMODJOBS/$JOB_ID/postprocs/$NODE_MEM/postprocinput.pl" ) {
   require "$HOMEDIR/data/GMODJOBS/$JOB_ID/postprocs/$NODE_MEM/postprocinput.pl";
} else {
   print " Required configuration file $NODE_MEM/postprocinput.pl is missing... exiting...\n";
   exit;
}
...
# set these for Ensemble Mean files...
   $model_fin_dir = "";
   $model_pre_dir = "";
   $model_out_name = "mean_d0";
 # ensemble mean output is in WRF-format...
   $IS_WRF = 1;


# Copy the file to the working dir to get a snapshot "now"
      foreach $d (1..$NDOMAINS) {
         $valid_time = &time_from_wrf_name("fill10char$SUFFIX");
if ( $CYCLE_TAG eq "final" ) {
	 $MM5_FILE = "${RUNDIR}/${THIS_CYCLE}/D${d}_${valid_time}_F/${model_out_name}${d}${SUFFIX}.nc";
} else {
	 $MM5_FILE = "${RUNDIR}/${THIS_CYCLE}/D${d}_${valid_time}_P+FCST/${model_out_name}${d}${SUFFIX}.nc";
}
            print "MM5_FILE = $MM5_FILE\n";
	 if ( -e $MM5_FILE ) {
...
if ( $CYCLE_TAG eq "final" ) {
  ...
  print "copying...$MM5_FILE to $WORK_FILE\n";
         $TEMP_F = "${WORK_DIR}/${WORK_FILE}";
         system("cp $MM5_FILE $TEMP_F");
  print "done...  to $TEMP_F\n";
	 if ( $IS_WRF ) {
           $valid_time = &time_from_wrf_name("fill10char$SUFFIX");
...
#	   $WRF_FILE_M1 = "wrfout_d0${d}_${yr}-${mo}-${dy}_${hr}:00:00";
	   $WRF_FILE_M1 = "mean_d0${d}_${yr}-${mo}-${dy}_${hr}:00:00.nc";
           $WORK_FILE_M1 = "${model_out_name}$d-1";
           # Check in the "working directory"... for previous hour's file
	   $HOURLY_FILE_M1 = "${RUNDIR}/postprocs/${WRF_FILE_M1}";
           if ( -e $HOURLY_FILE_M1 ) {
             system("cp $HOURLY_FILE_M1  $WORK_FILE_M1");
           }
           print (" Previous WRF file is: $HOURLY_FILE_M1\n");
...
require "$POSTPROCS_DIR/do_wrf2grib.pl";
...
if ( -e "$HOMEDIR/data/GMODJOBS/$JOB_ID/postprocs/$NODE_MEM/conv2gif_$RANGE.pl" ) {
 require "$HOMEDIR/data/GMODJOBS/$JOB_ID/postprocs/$NODE_MEM/conv2gif_$RANGE.pl";
} elsif ( -e "$POSTPROCS_DIR/conv2gif_$RANGE.pl" ) {
...
#          Copy the file to the web server for NAPS-interactive
           if ( $IS_WRF ) {
             &do_wrf2grib( $RUNDIR . "/postprocs/" . $OUT_FILED1, $MDV_DEST_DIR1);
           } else {
...
#          Copy the file to the web server for NAPS-interactive
           if ( $IS_WRF ) {
             &do_wrf2grib( $RUNDIR . "/postprocs/" . $OUT_FILED2, $MDV_DEST_DIR2);
           } else {
...
#          Copy the file to the web server for NAPS-interactive
           if ( $IS_WRF ) {
             &do_wrf2grib( $RUNDIR . "/postprocs/" . $OUT_FILED3, $MDV_DEST_DIR3);
           } else {
...
#          Copy the file to the web server for NAPS-interactive
           if ( $IS_WRF ) {
             &do_wrf2grib( $RUNDIR . "/postprocs/" . $OUT_FILED4, $MDV_DEST_DIR4);
           } else {
