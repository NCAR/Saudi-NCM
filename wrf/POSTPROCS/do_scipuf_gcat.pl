...
#
  ...
  $ASCII2MEDOC = "$MM5HOME/cycle_code/EXECUTABLE_ARCHIVE/ascii2medoc_d";
...
###TEST  &debug($DEBUG, "cleaning up $SCIPUF_DIR directory\n");
###TEST  system("rm -f ${SCIPUF_DIR}/*");
...
sub ascii_2_medoc
{
    ...
    system("cd ${SCIPUF_DIR}; ${ASCII2MEDOC} -L");
...
#LPC    system( "ssh $DEST_SERVER $move_command"); 
    system( "cd ${SCIPUF_DIR}; rsync -e 'ssh -i /data/GMODJOBS/$JOB_ID/rtfdda.key' -avzC ${medoc_file}.gz $DEST_SERVER:${dest}/" );
    }
    system( "cd ${SCIPUF_DIR}; cp ${medoc_file}.gz ${SCIPUF_ARCHIVE_DIR}" );
    system( "/home/fddasys/bin/scrub 1 ${SCIPUF_ARCHIVE_DIR}" );
   }
}
