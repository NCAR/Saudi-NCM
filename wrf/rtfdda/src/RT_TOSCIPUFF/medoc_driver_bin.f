...
!     integer kbuff(nk)
      ...
      if(j /= 1) then
        ...
        print*,'Usage: ',trim(exe_name),' -L (Lambert conformal) | -P (polar Stereographic)'
        stop
      endif
      ...
      elseif(proj == '-P') then
        ...
        print*,'Wrong projection specified (-L or -P only)'
        stop
      endif 

      open(10,file='list',status='old')
!
      cfg_file='scipuff.cfg'
!     inquire(file=cfg_file,exist=there)
!
      nfile1=11
!
...
!
         ...
         call wrtmedoc_bin(nx,ny,nz,iyear,imon,iday,ihour,imin,isec,ztrans, &
