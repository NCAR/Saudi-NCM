...
!
      ...
      real, allocatable, dimension(:,:,:) :: uue, vve, uve 
...
!
      ...
      allocate(u(nx,ny,nz))
      allocate(v(nx,ny,nz))
      allocate(uue(nx,ny,nz))
      allocate(vve(nx,ny,nz))
      allocate(uve(nx,ny,nz))
      allocate(w(nx,ny,nz))
...
!
         call readmm5(nfile1,filename,1,1,0,1,'UUE       ',nx,ny,nz,uue, &
         flag0)
         if(flag0 < 0) then
           call ConvertError(filename,flag0)
           stop
...
!
         call readmm5(nfile1,filename,1,1,0,1,'VVE       ',nx,ny,nz,vve, &
         flag0)
         if(flag0 < 0) then
           call ConvertError(filename,flag0)
           stop
...
!
         call readmm5(nfile1,filename,1,1,0,1,'UVE       ',nx,ny,nz,uve, &
         flag0)
         if(flag0 < 0) then
           call ConvertError(filename,flag0)
           stop
...
!  polar stereographic projection. Use 0.716 for Lambert conformal projection
         ...
         call model2sph(nx,ny,nz,xlonc,xlon,u,v,uue, vve, uve, buffer1,buffer2,xn)
!
...
!        write(6,'(6(f12.4,1x))') ((htflux(i,j),i=1,nx),j=1,ny)
         ...
         call hinterp3(nx,ny,nz,hfact,iindex,jindex,uue,buffer1)
         call hinterp3(nx,ny,nz,hfact,iindex,jindex,vve,buffer1)
         call hinterp3(nx,ny,nz,hfact,iindex,jindex,uve,buffer1)
         ...
         call zinterp(nx,ny,nz,slopef,kindex,uue,kbuff)
         call zinterp(nx,ny,nz,slopef,kindex,vve,kbuff)
         call zinterp(nx,ny,nz,slopef,kindex,uve,kbuff)
...
!
         ...
         call wrtmedoc(nx,ny,nz,iyear,imon,iday,ihour,imin,isec,ztrans, &
                       dlat,dlon,ylat,ylon,terr,u,v,uue, vve, uve, w,th,pblhgt,htflux, &
