...
! 5. julian_DAY (ny,nm,nd,jd,method)                                       
!    method = 1: input ---- ny, nm, nd.  output ---- jd 
!    method = 2: input ---- ny, jd.      output ---- nm, nd

! 6. sec_to_hhmmss(hh,mm,ss,seconds,method)
!     Conversion of hh,mm,ss to seconds (method=1) or
!                   seconds to hh,mm,ss (method=2).
!    
! 7. get_month (pmm,cdmm,order)
!    Conversion: 
!    if order = +1: month integer (pmm=1-12) into month string cdmm='january',..
!    if order = -1: string cdmm='january',...into month integer (pmm=1-12)
!
! 8. date_cat
!    Conversion of integers yy,mm,dd,hh,mn,ss into string ccyy-mm-dd)hh_mn:ss
!    (cc is guessed upon yy)

CONTAINS

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

   FUNCTION check_date (ndate) RESULT (npass)
   
      IMPLICIT NONE
      
      !  Check the consistency of an input date ('YYYY-MM-DD HH:MM:SS.ffff'), 
      
      !  on entry     -  ndate  -  the date string.
      
      !  on exit      -  npass    -  False if input string is not a date
      
      CHARACTER (LEN=*) , INTENT(INOUT) :: ndate
      LOGICAL                           :: npass
      ...
      IF ((monew.GT.12).or.(monew.LT.1)) THEN
         WRITE (UNIT = *, FMT = '(3A,I2)') &
        "Error in date= ", TRIM (ndate), " month= ", monew
         npass = .FALSE.
         RETURN
      END IF
      ...
      ! ...... For all months but February
         ...
         IF ((dynew.GT.mday(monew)).or.(dynew.LT.1)) THEN
              WRITE (UNIT = *, FMT = '(3A,I2)') &
             "Error in date= ", TRIM (ndate), " day= ", dynew
              npass = .false.
              RETURN
         END IF
      ...
      IF ((hrnew.GT.23).or.(hrnew.LT.0)) THEN
            WRITE (UNIT = *, FMT = '(3A,I2)') &
           "Error in date= ", TRIM (ndate), " hour= ", hrnew
            npass = .false.
            RETURN
      END IF
      ...
      IF ((minew.GT.59).or.(minew.LT.0)) THEN
           WRITE (UNIT = *, FMT = '(3A,I2)') &
          "Error in date= ", TRIM (ndate), " minute= ", minew
           npass = .false.
           RETURN
      END IF
      ...
      IF ((scnew.GT.59).or.(scnew.LT.0)) THEN
           WRITE (UNIT = *, FMT = '(3A,I2)') &
          "Error in date= ", TRIM (ndate), " second= ", scnew
           npass = .false.
           RETURN
      END IF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   SUBROUTINE geth_idts (ndate, odate, idts, pass)
      ...
      iout = 0
      IF (PRESENT (pass))  pass = .true.
      
      !  Check that the month of NDATE makes sense.
      
      IF ((monew.GT.12).or.(monew.LT.1)) THEN
           WRITE (UNIT = *, FMT = '(A,A)') 'Error in geth_idts:  Month of ndate = ', monew
           npass = .false.
      END IF
      ...
      IF ((moold.GT.12).or.(moold.LT.1)) THEN
           WRITE (UNIT = *, FMT = '(A,I6)') 'Error in geth_idts:  Month of odate = ', moold
           opass = .false.
      END IF
      ...
      ! ...... For all months but February
         IF ((dynew.GT.mday(monew)).or.(dynew.LT.1)) THEN
              WRITE (UNIT = *, FMT = '(A,I4)') 'Error in geth_idts:  Day of ndate = ', dynew
              npass = .false.
         END IF
      ELSE IF (monew.eq.2) THEN
      ! ...... For February
      ...
      ! ...... For all months but February
         IF ((dyold.GT.mday(moold)).OR.(dyold.LT.1)) THEN
              WRITE (UNIT = *, FMT = '(A,I4)') 'Error in geth_idts:  Day of odate = ', dyold
              opass = .false.
         END IF
      ...
      IF ((hrnew.GT.23).or.(hrnew.LT.0)) THEN
           WRITE (UNIT = *, FMT = '(A,I4)') 'Error in geth_idts:  Hour of ndate = ', hrnew
           npass = .false.
      END IF
      ...
      IF ((hrold.GT.23).or.(hrold.LT.0)) THEN
           WRITE (UNIT = *, FMT = '(A,I4)') 'Error in geth_idts:  hour of odate = ', hrold
           opass = .false.
      END IF
      ...
      IF ((minew.GT.59).or.(minew.LT.0)) THEN
           WRITE (UNIT = *, FMT = '(A,I4)') 'Error in geth_idts:  Minute of ndate = ', minew
           npass = .false.
      END IF
      ...
      IF ((miold.GT.59).or.(miold.LT.0)) THEN
           WRITE (UNIT = *, FMT = '(A,I4)') 'Error in geth_idts:  Minute of ODATE = ', miold
           opass = .false.
      END IF
      ...
      IF ((scnew.GT.59).or.(scnew.LT.0)) THEN
           WRITE (UNIT = *, FMT = '(A,I4)') 'Error in geth_idts:  Second of ndate = ', scnew
           npass = .false.
      END IF
      ...
      IF ((scold.GT.59).or.(scold.LT.0)) THEN
           WRITE (UNIT = *, FMT = '(A,I4)') 'Erro in geth_idts:  Second of odate = ', scold
           opass = .false.
      END IF
      ...
      IF (.not. npass) THEN
         IF (PRESENT (pass)) THEN
             idts = 0
             pass = .false.
             WRITE (UNIT = *, FMT = '(2A)') 'Screwy NDATE: ', ndate(1:nlen)
             RETURN
         ELSE
             STOP 'ndate_2'
         ENDIF
      ...
      IF (.not. opass) THEN
         IF (PRESENT (pass)) THEN
             idts = 0
             pass = .false.
             WRITE (UNIT = *, FMT = '(2A)') 'Screwy ODATE: ', odate(1:nlen)
             RETURN
         ELSE
             STOP 'odate_1'
         ENDIF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   SUBROUTINE geth_newdate (ndate, odate, idt)
      ...
      IF ((moold.GT.12).or.(moold.LT.1)) THEN
         WRITE (UNIT = 0, FMT = '(2A)') 'GETH_NEWDATE:  Month of ODATE = ', moold
         opass = .FALSE.
      END IF
      ...
      IF ((dyold.GT.mday(moold)).or.(dyold.LT.1)) THEN
         WRITE (UNIT = 0, FMT = '(2A)') 'GETH_NEWDATE:  Day of ODATE = ', dyold
         opass = .FALSE.
      END IF
      ...
      IF ((hrold.GT.23).or.(hrold.LT.0)) THEN
         WRITE (UNIT = 0, FMT = '(2A)') 'GETH_NEWDATE:  Hour of ODATE = ', hrold
         opass = .FALSE.
      END IF
      ...
      IF ((miold.GT.59).or.(miold.LT.0)) THEN
         WRITE (UNIT = 0, FMT = '(2A)') 'GETH_NEWDATE:  Minute of ODATE = ', miold
         opass = .FALSE.
      END IF
      ...
      IF ((scold.GT.59).or.(scold.LT.0)) THEN
         WRITE (UNIT = 0, FMT = '(2A)') 'GETH_NEWDATE:  Second of ODATE = ', scold
         opass = .FALSE.
      END IF
      ...
      !KWM         WRITE (UNIT = 0, FMT = '(2A)') 'GETH_NEWDATE:  Second of ODATE = ', scold
      !KWM         opass = .FALSE.
      !KWM      END IF
      
      IF (.not.opass) THEN
         WRITE (UNIT = 0, FMT = '(3A)') 'GETH_NEWDATE: Crazy ODATE: ', odate(1:olen), olen
         STOP 'odate_3'
      END IF
      ...
      ELSE IF (olen.eq.10) THEN !idt should be in days
         ...
         nfrac  = 0
      ELSE
         WRITE (UNIT = 0, FMT = '(''GETH_NEWDATE: Strange length for ODATE: '', i3)') &
              olen
         WRITE (UNIT = 0, FMT = '(2A)') odate(1:olen)
         STOP 'odate_4'
      END IF
...
!    
  ...
  if (method == 1) then
    ...
    WRITE (UNIT = 0, FMT = '(''Method ='',I3,'' is invalid'')') method
  endif

end subroutine Sec_to_hhmmss

SUBROUTINE get_month (mm,month,order)

! Convert month from string to digit (order > 0) and vice versa (order < 0) 
! Output string is always full name (eg JANUARY, FEBRUARY) in capital letters.
! Input  string can be either full name (eg JANUARY, FEBRUARY) or the first
! 3 letters (JAN, FEB). Both must be in capital letters.
      ...
      IF (order > 0) THEN
          ...
          SELECT CASE (mm)
                 ...
                 CASE DEFAULT ; month = 'UNKNOWN'
    
          END SELECT
    
      ELSE
    
          IF (LEN_TRIM (month) > 3) THEN
...
CHARACTER (LEN = 19) FUNCTION date_cat (yy,mm,dd,hh,mn,ss)

!------------------------------------------------------------------------------!
      IMPLICIT NONE

      INTEGER, INTENT (in) :: yy,mm,dd,hh,mn,ss
!     CHARACTER (LEN = 19) :: date_cat
      INTEGER              :: cc
!------------------------------------------------------------------------------!

! Guess the century based on the year
...
!...date as ccyy-mm-dd_hh:mn:ss

      WRITE (date_cat,'(2(I2.2),"-",I2.2,"-",I2.2,"_",I2.2,":",I2.2,":",I2.2)')&
             cc,yy,mm,dd,hh,mn,ss

END FUNCTION date_cat
