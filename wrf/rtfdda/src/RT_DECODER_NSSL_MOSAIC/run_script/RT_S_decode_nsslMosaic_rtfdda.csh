...
if(-d GEAPSTMP_tmp) then
 if( ! -d $GEAPSTMP/DECODE_NSSL_MOSAIC) then
  mkdir -p $GEAPSTMP/DECODE_NSSL_MOSAIC
 endif
 cd $GEAPSTMP/DECODE_NSSL_MOSAIC 
 echo "Current directory is $GEAPSTMP/DECODE_NSSL_MOSAIC "
else
 if (! -d $RUNDIR/${this_cycle}/DECODE_NSSL_MOSAIC) mkdir -p $RUNDIR/${this_cycle}/DECODE_NSSL_MOSAIC
...
advance_cymdh.exe < input >! output
