!=======================================================================
! CVS: $Id: WRF_ncread.f90,v 1.7 2018/01/30 21:30:37 sheu Exp $
! CVS: $Source: /cvs/apps/netcdf_utils/src/WRF_to_point_data/WRF_ncread.f90,v $
! CVS: $Name:  $
!=======================================================================
!BOP ===================================================================
!
...
! !PUBLIC MEMBER FUNCTIONS:
   ...
   public :: ncread_close       ! Close a netCDF file
...
!EOP
   ...
   logical, parameter :: debug_io = .false. ! Set to .true. to turn on IO debugging
   character(char_long), parameter :: def_cache_fileName  = "_N/A_" ! default file name
   character(char_long) :: cached_fileName = def_cache_fileName ! cached file name
   integer(INT) :: cached_fid = -1, cached_fid_count = 0
   integer(INT) :: cached_rCode = 0
   character(char_long) :: cached_fileName2 = def_cache_fileName ! cached file name
   integer(INT) :: cached_fid2 = -1, cached_fid_count2 = 0
   integer(INT) :: cached_rCode2 = 0
...
!EOP
   ...
   !if (debug_io) print *, "fileName:", trim(fileName), " cached_fileName:", trim(cached_fileName)

   if (trim(fileName) .eq. trim(cached_fileName)) then
      fid = cached_fid
      cached_fid_count = cached_fid_count + 1
   else if (trim(fileName) .eq. trim(cached_fileName2)) then
      fid = cached_fid2
      cached_fid_count2 = cached_fid_count2 + 1
   else
      if (debug_io) call cpu_time(start_time)
      rCode = nf90_open(fileName, nf90_nowrite, fid)
      if (debug_io) then
         call cpu_time(stop_time)
         print *, "ncread_open time:", stop_time - start_time, "seconds"
      end if
      call handleErr(rCode, subName//" ERROR opening input data file")
      if (trim(def_cache_fileName) .eq. trim(cached_fileName)) then
         cached_fileName = fileName
         cached_fid = fid
         cached_fid_count = 1
         cached_rCode = rCode
         if (debug_io) print *, "ncread_open Cashed the first file: ", trim(fileName)," ", fid
      else if (trim(def_cache_fileName) .eq. trim(cached_fileName2)) then
         cached_fileName2 = fileName
         cached_fid2 = fid
         cached_fid_count2 = 1
         cached_rCode2 = rCode
         if (debug_io) print *, "ncread_open Cashed the second file: ", trim(fileName)," ", fid
      end if
   end if

end subroutine ncread_open

function ncread_close(fid, subName)
...
!EOP
   ...
   !----- local -----
   logical :: close_file ! Set to .true. to close NetCDF
   integer(INT)   :: rCode
   integer(INT)   :: ncread_close
   
   close_file = .true.
!   if (debug_io) print *, "ncread_close is called file handler: ", fid
   if (cached_fid .eq. fid) then
      cached_fid_count = cached_fid_count - 1
      if (cached_fid_count > 0) close_file = .false.
      rCode = cached_rCode
   else if (cached_fid2 .eq. fid) then
      cached_fid_count2 = cached_fid_count2 - 1
      if (cached_fid_count2 > 0) close_file = .false.
      rCode = cached_rCode2
   end if   
   if (close_file) then
      rCode = nf90_close(fid)
      call handleErr(rCode, subName//" ERROR closing input data file")
      if (debug_io) print *, "ncread_close Closed the file handler %i", fid
      if (cached_fid .eq. fid) then
         if (debug_io) print *, "ncread_close Closed the first cached file: ", trim(cached_fileName)
         cached_fid = -1
         cached_fileName = def_cache_fileName
      else if (cached_fid2 .eq. fid) then
         cached_fid2 = -1
         if (debug_io) print *, "ncread_close Closed the second cached file: ", trim(cached_fileName2)
         cached_fileName2 = def_cache_fileName
      end if
   endif
   ncread_close = rCode
end function ncread_close
...
!   character(*),parameter :: F00 = "('(get_gl_int_att) ',3a,f10.4)"
   character(*),parameter :: F00 = "('(get_gl_int_att) ',3a,i10)"
...
!EOP
   ...
   if (debug_io) then
      call cpu_time(stop_time)
      print *, "dimension time:", stop_time - start_time, "seconds"
   end if
...
!-------------------------------------------------------------------------------
   ...
   if (debug) write(6,F01) trim(varName)//' has dims = ',ns
...
!EOP
   ...
   if (rCode /= nf90_noerr) then
    ! write(6,F00) "netCDF error: ",trim(nf90_strerror(rCode))
      write(6,F00) "netCDF error: ",trim(str)
      call flush(6)
      STOP
   end if
