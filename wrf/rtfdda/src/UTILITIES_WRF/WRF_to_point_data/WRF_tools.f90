!=======================================================================
! CVS: $Id: WRF_tools.f90,v 1.5 2017/05/19 22:24:11 sheu Exp $
! CVS: $Source: /cvs/apps/netcdf_utils/src/WRF_to_point_data/WRF_tools.f90,v $
! CVS: $Name:  $
!=======================================================================
!BOP ===================================================================
!
...
! !PUBLIC MEMBER FUNCTIONS:
   ...
   public :: tdcalc           ! Function to calculate dew-point temperature
   public :: unstagger        ! Average staggered fields to unstaggered grid
   public :: upper            ! Function to convert string to uppercase
...
!EOP
   ...
   if( map_proj .eq. 2) then      ! Polar stereographic
      cone = c1
   endif
   if( map_proj .eq. 3) then      ! Mercator
      cone = 0.
   endif
...
!EOP
   ...
   if (nx == nWE_stag) then  
      unstaggered = half*(staggered(1:nx-1,:,:) + staggered(2:nx,:,:))
   endif

   if (ny == nSN_stag) then  
      unstaggered = half*(staggered(:,1:ny-1,:) + staggered(:,2:ny,:))
   endif
