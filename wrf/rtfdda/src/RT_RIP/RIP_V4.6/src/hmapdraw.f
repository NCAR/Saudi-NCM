...
c         write(iup,*) 'To use the RG data set you must have'
c         write(iup,*) 'NCAR Graphics Version 4.3.  If you have it,'
c         write(iup,*) 'comment out this print statement and the'
c         write(iup,*) 'subsequent stop statement in routine hmapdraw.,'
c         write(iup,*) 'and uncomment the following section of code.'
c         write(iup,*) 'Then recompile and rerun rip.'
c         stop
         if (couty(ipl)(3:3).ne.' ') then
            read(couty(ipl)(3:3),'(i1)') irgres  ! 0 (finest) to 4 (coarsest)
         else
            call mdrgdl(irgres) ! Get suggested resolution of RANGS/GSHHS data.
         endif
         call mpseti('II',64064)
         if (mpfillco(1).ne.999999) then
            do i=1,5
               icol(i)=iouco(ipl)
               icsf(i)=mpfillco(i)
            enddo
            call mdrgsc(icol,icsf)
            call mdrgsf (irgres,xcra,ncra,iama,nama)
            call mdrgol (irgres,xcra,ncra)
         else
            do i=1,5
               icol(i)=iouco(ipl)
               icsf(i)=iouco(ipl)
