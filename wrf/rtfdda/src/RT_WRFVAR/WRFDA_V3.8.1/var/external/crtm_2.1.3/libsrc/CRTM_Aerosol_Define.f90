...
MODULE CRTM_Aerosol_Define
  ...
  '$Id: CRTM_Aerosol_Define.f90,v 1.1 2017/06/30 19:53:21 sheu Exp $'
...
!       a CRTM Aerosol object to stdout.
!
! CALLING SEQUENCE:
!       CALL CRTM_Aerosol_MaxInspect( Aerosol )
!
! INPUTS:
!       Aerosol:       CRTM Aerosol object query and display.
!                      UNITS:      N/A
!                      TYPE:       CRTM_Aerosol_type
!                      DIMENSION:  Scalar, Rank-1, or Rank-2 array
!                      ATTRIBUTES: INTENT(IN)
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Scalar_MaxInspect( Aerosol )
    TYPE(CRTM_Aerosol_type), INTENT(IN) :: Aerosol
    INTEGER :: lType
    ...
    lType = Aerosol%Type
    IF ( lType < 1 .OR. lType > N_VALID_AEROSOL_TYPES ) lType = INVALID_AEROSOL
    WRITE(*, '(3x,"Type     :",1x,a)') TRIM(AEROSOL_TYPE_NAME(lType))
    IF ( .NOT. CRTM_Aerosol_Associated(Aerosol) ) RETURN
    ! Profile information
    WRITE(*, '(3x,"Max. effective radius: ",es13.6)') MaxVal_Find(Aerosol%Effective_Radius)
    WRITE(*, '(3x,"Max. concentration   : ",es13.6)') MaxVal_Find(Aerosol%Concentration)
  END SUBROUTINE Scalar_MaxInspect

  SUBROUTINE Rank1_MaxInspect( Aerosol )
    TYPE(CRTM_Aerosol_type), INTENT(IN) :: Aerosol(:)
    INTEGER :: i
    DO i = 1, SIZE(Aerosol)
      WRITE(*, FMT='(1x,"RANK-1 INDEX:",i0," - ")', ADVANCE='NO') i
      CALL Scalar_MaxInspect(Aerosol(i))
    END DO
  END SUBROUTINE Rank1_MaxInspect

  SUBROUTINE Rank2_MaxInspect( Aerosol )
    TYPE(CRTM_Aerosol_type), INTENT(IN) :: Aerosol(:,:)
    INTEGER :: i, j
    DO j = 1, SIZE(Aerosol,2)
      DO i = 1, SIZE(Aerosol,1)
        WRITE(*, FMT='(1x,"RANK-2 INDEX:",i0,",",i0," - ")', ADVANCE='NO') i,j
        CALL Scalar_MaxInspect(Aerosol(i,j))
      END DO
    END DO
...
!
! PURPOSE:
!       Subroutine to compare two Aerosol objects and print the MAXIMUM
!       differences to stdout.
!
! CALLING SEQUENCE:
!       CALL CRTM_Aerosol_Inspect( Aerosol1, Aerosol2, n_SigFig=n_SigFig )
!
! INPUTS:
!       Aerosol1, Aerosol2:  The CRTM Aerosol objects to compare and display.
!                            UNITS:      N/A
!                            TYPE:       CRTM_Aerosol_type
!                            DIMENSION:  Scalar, Rank-1, or Rank-2 array
!                            ATTRIBUTES: INTENT(IN)
!
! OPTIONAL INPUTS:
!       n_SigFig:            Number of significant figure to compare floating
!                            point components.
!                            UNITS:      N/A
!                            TYPE:       INTEGER
!                            DIMENSION:  Scalar
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Scalar_CompareInspect( x, y, n_SigFig )
    ...
    ! Local variables
    INTEGER :: xlType, ylType
    INTEGER :: n
    REAL(fp) :: rxval, ryval, rdiff

    IF ( CRTM_Aerosol_Compare(x, y, n_SigFig=n_SigFig) ) RETURN
    ...
    WRITE(*, '(1x,"Aerosol OBJECT CompareInspect")')
    WRITE(*, '(3x,"n_Layers :",1x,i0,1x,i0)') x%n_Layers, y%n_Layers
    xlType = x%Type
    IF ( xlType < 1 .OR. xlType > N_VALID_AEROSOL_TYPES ) xlType = INVALID_AEROSOL
    ylType = y%Type
    IF ( ylType < 1 .OR. ylType > N_VALID_AEROSOL_TYPES ) ylType = INVALID_AEROSOL
    WRITE(*, '(3x,"Type     :",1x,a,", ",a)') &
             TRIM(AEROSOL_TYPE_NAME(xlType)), &
             TRIM(AEROSOL_TYPE_NAME(ylType))


    IF ( x%n_Layers /= y%n_Layers ) &
      WRITE(*, '(3x,"n_Layers   :",1x,i0,1x,i0)') x%n_Layers, y%n_Layers
    IF ( x%Type /= y%Type ) &
      WRITE(*, '(3x,"Type index :",1x,i0,1x,i0)') x%Type, y%Type


    IF ( (.NOT. ALL(Compares_Within_Tolerance(x%Effective_Radius ,y%Effective_Radius ,n))) ) THEN
      CALL MaxDiff_Find(x%Effective_Radius, y%Effective_Radius, rxval, ryval, rdiff)
      WRITE(*,'(3x,"Effective_Radius difference:")')
      WRITE(*,'(5x,"aerosol1 : ",es22.15)') rxval
      WRITE(*,'(5x,"aerosol2 : ",es22.15)') ryval
      WRITE(*,'(5x,"diff   : ",es22.15)') rdiff
    END IF
    IF ( (.NOT. ALL(Compares_Within_Tolerance(x%Concentration ,y%Concentration ,n))) ) THEN
      CALL MaxDiff_Find(x%Concentration, y%Concentration, rxval, ryval, rdiff)
      WRITE(*,'(3x,"Concentration difference:")')
      WRITE(*,'(5x,"aerosol1 : ",es22.15)') rxval
      WRITE(*,'(5x,"aerosol2 : ",es22.15)') ryval
      WRITE(*,'(5x,"diff   : ",es22.15)') rdiff
    END IF
  ...
  SUBROUTINE Rank1_CompareInspect( x, y, n_SigFig )
    ...
    INTEGER,       OPTIONAL, INTENT(IN) :: n_SigFig
    INTEGER :: i
    IF ( ALL(CRTM_Aerosol_Compare(x, y, n_SigFig=n_SigFig)) ) RETURN
    WRITE(*, '(1x,"Aerosol RANK-1 ARRAY CompareInspect")')
    DO i = 1, SIZE(x)
      WRITE(*, FMT='(1x,"Profile:",i0)') i
  ...
  SUBROUTINE Rank2_CompareInspect( x, y, n_SigFig )
    ...
    INTEGER :: i,j
    IF ( ALL(CRTM_Aerosol_Compare(x, y, n_SigFig=n_SigFig)) ) RETURN
    WRITE(*, '(1x,"Aerosol RANK-2 ARRAY CompareInspect")')
    DO j = 1, SIZE(x,DIM=2)
      DO i = 1, SIZE(x,DIM=1)
        WRITE(*, FMT='(1x,"Channel:",i0,", Profile:",i0)') i,j
