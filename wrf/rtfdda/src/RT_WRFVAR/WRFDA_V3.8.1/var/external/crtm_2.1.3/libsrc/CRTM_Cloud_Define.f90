...
MODULE CRTM_Cloud_Define
  ...
  '$Id: CRTM_Cloud_Define.f90,v 1.1 2017/06/30 19:53:21 sheu Exp $'
...
!       a CRTM Cloud object to stdout.
!
! CALLING SEQUENCE:
!       CALL CRTM_Cloud_Inspect( Cloud )
!
! INPUTS:
!       Cloud:         CRTM Cloud object query and display.
!                      UNITS:      N/A
!                      TYPE:       CRTM_Cloud_type
!                      DIMENSION:  Scalar, Rank-1, or Rank-2 array
!                      ATTRIBUTES: INTENT(IN)
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Scalar_MaxInspect( Cloud )
    TYPE(CRTM_Cloud_type), INTENT(IN) :: Cloud
    INTEGER :: lType
    ...
    IF ( .NOT. CRTM_Cloud_Associated(Cloud) ) RETURN
    ! Profile information
    WRITE(*, '(3x,"Max. effective radius: ",es13.6)') MaxVal_Find(Cloud%Effective_Radius)
    WRITE(*, '(3x,"Max. water content   : ",es13.6)') MaxVal_Find(Cloud%Water_Content)
  END SUBROUTINE Scalar_MaxInspect

  SUBROUTINE Rank1_MaxInspect( Cloud )
    TYPE(CRTM_Cloud_type), INTENT(IN) :: Cloud(:)
    INTEGER :: i
    DO i = 1, SIZE(Cloud)
      WRITE(*, FMT='(1x,"RANK-1 INDEX:",i0," - ")', ADVANCE='NO') i
      CALL Scalar_MaxInspect(Cloud(i))
    END DO
  END SUBROUTINE Rank1_MaxInspect

  SUBROUTINE Rank2_MaxInspect( Cloud )
    TYPE(CRTM_Cloud_type), INTENT(IN) :: Cloud(:,:)
    INTEGER :: i, j
    DO j = 1, SIZE(Cloud,2)
      DO i = 1, SIZE(Cloud,1)
        WRITE(*, FMT='(1x,"RANK-2 INDEX:",i0,",",i0," - ")', ADVANCE='NO') i,j
        CALL Scalar_MaxInspect(Cloud(i,j))
      END DO
    END DO
...
!
! PURPOSE:
!       Subroutine to compare two Cloud objects and print the MAXIMUM differences
!       to stdout.
!
! CALLING SEQUENCE:
!       CALL CRTM_Cloud_Inspect( Cloud1, Cloud2, n_SigFig=n_SigFig )
!
! INPUTS:
!       Cloud1, Cloud2:  The CRTM Cloud objects to compare and display.
!                        UNITS:      N/A
!                        TYPE:       CRTM_Cloud_type
!                        DIMENSION:  Scalar, Rank-1, or Rank-2 array
!                        ATTRIBUTES: INTENT(IN)
...
!--------------------------------------------------------------------------------
  ...
  SUBROUTINE Scalar_CompareInspect( x, y, n_SigFig )
    ...
    IF ( CRTM_Cloud_Compare(x, y, n_SigFig=n_SigFig) ) RETURN
    ...
    WRITE(*, '(1x,"Cloud OBJECT CompareInspect")')
    WRITE(*, '(3x,"n_Layers :",1x,i0,1x,i0)') x%n_Layers, y%n_Layers
    xlType = x%Type
    IF ( xlType < 1 .OR. xlType > N_VALID_CLOUD_TYPES ) xlType = INVALID_CLOUD
    ylType = y%Type
    IF ( ylType < 1 .OR. ylType > N_VALID_CLOUD_TYPES ) ylType = INVALID_CLOUD
    WRITE(*, '(3x,"Type     :",1x,a,", ",a)') &
             TRIM(CLOUD_TYPE_NAME(xlType)), &
             TRIM(CLOUD_TYPE_NAME(ylType))
    ...
    IF ( (.NOT. ALL(Compares_Within_Tolerance(x%Effective_Radius ,y%Effective_Radius ,n))) ) THEN
      ...
      WRITE(*,'(5x,"cloud1 : ",es22.15)') rxval
      WRITE(*,'(5x,"cloud2 : ",es22.15)') ryval
      WRITE(*,'(5x,"diff   : ",es22.15)') rdiff
    END IF
    IF ( (.NOT. ALL(Compares_Within_Tolerance(x%Water_Content ,y%Water_Content ,n))) ) THEN
      CALL MaxDiff_Find(x%Water_Content, y%Water_Content, rxval, ryval, rdiff)
      WRITE(*,'(3x,"Water_Content difference:")')
      WRITE(*,'(5x,"cloud1 : ",es22.15)') rxval
      WRITE(*,'(5x,"cloud2 : ",es22.15)') ryval
      WRITE(*,'(5x,"diff   : ",es22.15)') rdiff
    END IF
  ...
  SUBROUTINE Rank1_CompareInspect( x, y, n_SigFig )
    ...
    INTEGER,     OPTIONAL, INTENT(IN) :: n_SigFig
    INTEGER :: i
    IF ( ALL(CRTM_Cloud_Compare(x, y, n_SigFig=n_SigFig)) ) RETURN
    WRITE(*, '(1x,"Cloud RANK-1 ARRAY CompareInspect")')
    DO i = 1, SIZE(x)
  ...
  SUBROUTINE Rank2_CompareInspect( x, y, n_SigFig )
    ...
    INTEGER :: i,j
    IF ( ALL(CRTM_Cloud_Compare(x, y, n_SigFig=n_SigFig)) ) RETURN
    WRITE(*, '(1x,"Cloud RANK-2 ARRAY CompareInspect")')
    DO j = 1, SIZE(x,DIM=2)
      DO i = 1, SIZE(x,DIM=1)
