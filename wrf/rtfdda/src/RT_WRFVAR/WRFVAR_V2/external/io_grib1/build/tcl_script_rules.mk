...
#
#      include $(BUILD_DIR)/tcl_script_rules.mk
#
...
lib:
	...
	pwd=`pwd` ; \
	if [ -f $(MOD_DEST)/$(MOD_NAME) ]; then \
		echo "    Removing $(MOD_DEST)/$(MOD_NAME)"; \
		rm -f $(MOD_DEST)/$(MOD_NAME); \
	fi; \
	for s in $${src}; do \
		if [ "$${s}" = "invalid" ]; then \
			continue;\
		fi;\
		echo "    Appending $$s to $(MOD_DEST)/$(MOD_NAME)" ; \
		cat $$s >> $(MOD_DEST)/$(MOD_NAME) ; \
		if [ $$? != 0 ]; then \
		   echo "Error with cat $$s"; \
		fi; \
		ext=`echo $$s | sed -e 's/.*\.//'` ;\
	done; \
	chmod 444 $(MOD_DEST)/$(MOD_NAME) ; \
	cd $(MOD_DEST) ; \
