...
c        ... Output the particularly good obs.
     ...
     *             elev,t,spd,dir,gust,slp,altim,td,rh,ifirst)
            ...
            imm=ihhmm - (ihhmm/100)*100            ! YLIU
            kx=1
            iseq_num=istationkeep 
            write (staid,fmt='("9",i4.4,1x,"METNET=",i2)' )  !YLIU
     *            istationkeep,metnet
            call write_obs (p,z,t,td,spd,dir, 
     *                 slp, ter, xlat, xlon, mdate, imm, kx,  !YLIU
     *      staid(1:15) //'                         ',  
...
10    format(a3)
      ...
      print *,statest,' ',idate,itime,ifacility
      return
      end
...
c     endif
      ...
      write (6,1010) iyymmdd,ihhmm,metnet,staname,xlat,xlon,elev,
     *               t,td,spd,dir,slp
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     *             // ' 2i10 , a20 ,  13( f13.5 , i7 ) ) '
      ...
      write (date_char(9:16),fmt='(i8.8)') mdate  ! YLIU
      write (date_char(17:18),fmt='(i2.2)') imm   ! YLIU
