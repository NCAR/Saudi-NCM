  ...
  character (len=10) :: atime_new,atime_old='aaaaaaaaaa',mm5_time='0000000000'
  integer :: t_count
  ...
  if(j > 0) then
    call getarg(1,opt)
    if(opt /= '-f' .and. opt /= '-p') then
      print*,'Flag ',opt,' unknown! Should be either -p, -f or no flag'
      stop
    endif
  endif
!
...
SND:  do while(ierr == 0)
     ...
     read(snd_unit,*,IOSTAT=ierr) atime,st_id,alat,alon,elev,n_levels
     if(ierr /= 0) exit SND
...
!
     if(atime_new == mm5_time) then
       found=.TRUE.
       early=.FALSE.
     endif

     if(atime_new /= atime_old .and. atime_new > mm5_time) then      !! ready to read new MM5 data
       ...
       if(icnt > 0) then
         ...
         if(allocated(pstr)) deallocate(pstr)
...
!      write(out_unit,'(a10,f7.2,f8.2,i2)') atime_new, alat, alon, domain_id
       ...
       irec=irec+1
       write(out_unit,rec=irec) atime_new, st_id, alat, alon, elev, domain_id
