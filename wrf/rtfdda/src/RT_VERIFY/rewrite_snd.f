            ...
            do i=1,6
               if(measure%obs(i)%qc_flag >= 32768) then
                 if(measure%obs(i)%qc_flag == 400000) then
                   ...
                   qc=measure%obs(i)%qc_flag-int(measure%obs(i)%qc_flag/10)*10
...
!      if(index(description,'PROFILER') == 0 .and. &
       if(index(description,'SATWNDS') == 0 .and. &
         ...
         write(61,'(a14,x,a8,f8.3,f9.3,i10,i4)') time_str,st_id,lat,lon,elev,n_levels

         do while (associated(LevelList))
            write(61,'(i7,i7,5(i6,i7))') LevelList%data
