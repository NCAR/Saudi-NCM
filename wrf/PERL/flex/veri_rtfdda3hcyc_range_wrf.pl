...
$web_images_rel = "/images/ops/$range/rtfdda/veri_images";  ######
...
#
##system("rsync -e 'ssh -i $KEY' -avzC $date $WebServer:$web_images_dir/sfc/.");
...
##    system("rsync -e 'ssh -i $KEY' -avzC $UPR_STATS_TMP/$upr_dir $WebServer:$web_images_dir/upr/.");
...
# yliu
  ...
  system("$CSH_ARCHIVE/Forecast/RT_all.obs_cleanup.pl -f qc_out.dat");
...
### Forecast!  
  ...
  $fn=$$r_old_cycles[7]."_veri_dat_${RANGE_tag}_P+FCST";
...
";
  close(OUT);
##system("rsync -e 'ssh -i $KEY' -avzC $fn $WebServer:$web_images_dir/sfc/$date/.");
  unlink "$fn";
}
...
sub save_upr {
  ...
  system("$CSH_ARCHIVE/Forecast/RT_all.obs_cleanup.pl -S -f qc_out.dat");
