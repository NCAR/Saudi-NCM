...
#use lib '$PERL_ARCHIVE';
#use Dirs;
#use Time;
require  $PERL_ARCHIVE."/Dirs.pm";
require  $PERL_ARCHIVE."/Time.pm";
...
if (defined($VERI_LENGTH) && ($VERI_LENGTH > 0)) {
   print "VERIFY $VERI_LENGTH hours of forecast!\n";
} else {
   $VERI_LENGTH = 24;
}

$numOldCycles = $VERI_LENGTH/$CYC_INT;
...
($file_final,$file_fcst)=save_sfc(\@old_cycles); ## needs to be uncommented out later!
...
unlink <stats_*>; ## needs to be uncommented out later

# Remove the picture directory after it's remotely copied to $WebServer

##unlink <$date/*>; ## needs to be uncommented out later
##rmdir $date;      ## needs to be uncommented out later
...
#   $domain=2;
   ...
   if($PROD{"plot"} eq "yes") {
    foreach $fn (@stats_upr_file) {
      ...
      print "cat $html_table >> $UPR_STATS_TMP/$upr_dir/$html_doc\n";
      system("cat $html_table >> $UPR_STATS_TMP/$upr_dir/$html_doc");
      if ($DEST_SERVER=~ /localhost/) {
         system("mkdir -p $JOB_LOC/veri_images/upr/$upr_dir");
         system("cp $UPR_STATS_TMP/$upr_dir/* $JOB_LOC/veri_images/upr/$upr_dir/.");
      } else {
...
#
if($PROD{"plot"} eq "yes") {  
  @dirs=`find $UPR_STATS_TMP/. -type d`;
  ...
  foreach $dir (@dirs) {
    ...
    $flag_delete=old_enough($date,$tag);
    if($flag_delete == 1) {
      print "Removing $dir from $UPR_STATS_TMP!\n";
      system("rm -rf $dir");
    }
  }
}
...
### Forecast!  
  if($MMOUTDIR eq $RUNDIR) {
    ...
    print "Cycle $VERI_LENGTH hours ago: $cycOldest\n";
  ...
  $fn = $cycOldest . "_veri_dat_${RANGE_tag}_P+FCST";
...
if($PROD{"3dvar"} eq "yes") {   #--------
  ...
  @dates=();
  foreach $wrfout (<$RUNDIR/$new_dir/wrfout*P+FCST>) {
     chomp($wrfout);
     $wrfout=~ /d(\d+)_(\d{4}-\d{2}-\d{2}_\d{2})/;
     $dateString=$2;
     $datePush=check_element(\@dates,$dateString);
  ...
  if(-e $new_dir) {
    foreach $datetime (@dates) {
      system("rm -f pairs_domain*");
      $wrf="$new_dir/wrfout_d01_${datetime}:00:00.${RANGE}_3DVAR";
      print "--- $wrf\n";
      system("$EXECUTABLE_ARCHIVE/v_wrf_sfc_interp.exe $wrf") if(-e $wrf);
      system("cat pairs_domain1 >> $fn");
    }
  }
 
##system("rsync -e 'ssh -i $KEY' -avzC pairs_swap $WebServer:$REMOTE_DIR_SFC/fcst/$fn");
  system("mv $fn $SAVE_DIR_SFC/3dvar/$fn");
...
# print "$new_dir\n";
  ...
  if(-e $new_dir) {
    foreach $datetime (@dates) {
      system("rm -f pairs_domain*");
      $wrf="$new_dir/wrfout_d01_${datetime}:00:00.${RANGE}$FDDAT";
      system("$EXECUTABLE_ARCHIVE/v_wrf_sfc_interp.exe $wrf") if(-e $wrf);
      system("cat pairs_domain1 >> $fn");
    }
  }
...
##system("rsync -e 'ssh -i $KEY' -avzC pairs_swap $WebServer:$REMOTE_DIR_SFC/fcst/$fn");
  system("mv $fn $SAVE_DIR_SFC/fdda/$fn");
...
#
  ...
  foreach $file (@files) {
    ...
    if($file=~ /_F$/) {
      ...
      $time_min=date_retro($time_tag,$VERI_LENGTH); 
      ...
      $time_max=advance_h($time_tag,$VERI_LENGTH-1);
      $color='/255/0/0';
    }
    ...
    if($n == 1) {
      ...
      open(PIPE,"| psxy -JX7.32/2 -R0/${VERI_LENGTH}.4/$b_RANGE -Bf1a6:.\"$title\":/$ytick:$yaxis: -M -W1.5p$color -X1 -Y7.5 -K > $outf");
    } else {
...
# Annotate
  ...
  if ($time_stamp_fcst) {
     open(PIPE,"| psxy -JX7/0.25 -R0/${VERI_LENGTH}.4/0/10 -Y-0.75 -W1.5p/255/0/0 -O -K >> $outf");
     $xposb = 0;
     $xpose = $VERI_LENGTH * 0.3;
     print PIPE "$xposb 5 \n $xpose 5";
     close(PIPE);

     open(PIPE,"| psxy -JX -R -W1.5p/0/255/0 -O -K >> $outf");
     $xposb = $VERI_LENGTH*2/3;
     $xpose = $VERI_LENGTH*0.9667;
     print PIPE "$xposb 5 \n $xpose 5";
     close(PIPE);
  } else {
     open(PIPE,"| psxy -JX7/0.25 -R0/${VERI_LENGTH}.4/0/10 -Y-0.75 -W1.5p/0/255/0 -O -K >> $outf");
     $xposb = $VERI_LENGTH*2/3;
     $xpose = $VERI_LENGTH*0.9667;
     print PIPE "$xposb 5 \n $xpose 5";
     close(PIPE);
  ...
  $xpos = $VERI_LENGTH*0.15;
  print PIPE "$xpos 0 12 0 0 MC $time_stamp_fcst FCST\n";
  close(PIPE);
  }

  open(PIPE,"| pstext -JX -R -N -O >> $outf");
  $xpos = $VERI_LENGTH*0.81667;
  ...
  system("convert -trim +repage -density 56 $outf $out_gif");
...
sub save_upr {
  ...
  if(defined($VERI_HGT) && ($VERI_HGT > 0)) {
...
### Final!
  ...
  foreach $wrfout (<$RUNDIR/$date/wrfout*F>) {
     ...
     # skip hours not equal to 0, 6, 12, 18
# remove this for 6-hr cycling verification of non-synoptic obs
...
#  print "this_cycle = $this_cycle; final; skip = $skip\n";
  ...
  foreach $datetime (@dates) {
    foreach $d (@ds) {
      ...
      if(defined($VERI_HGT) && ($VERI_HGT > 0)) {
        system("$EXECUTABLE_ARCHIVE/v_wrf_snd_pairs.exe $wrf -height");
      } else {
...
#  print "this_cycle = $this_cycle; fcst; skip = $skip\n";
  ...
  if(-e $new_dir) {
    foreach $datetime (@dates) {
      ...
      undef @split_files;
      @split_files=split_upr('fort.81',$cycOldest,'FCST');
      unlink 'fort.81';
      foreach $f (@split_files) {
##      system("rsync -e 'ssh -i $KEY' -avzC $f $WebServer:$REMOTE_DIR_UPR/fcst/.");
  ...
  print "   Finish forecast-_upr of $cycOldest  at ",&ctime(time);
