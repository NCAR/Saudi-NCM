...
sub processData
{
  ...
  if($BCS_CHANGE == 1 || ($BCS_CHANGE != 1 && $BCS_date == $BCS_date_L))
  {
    ...
    {
      $ii = sprintf("%02d", $i);
      $BCS_file = "eta.T".substr($BCS_date,8,2)."Z.AWIP3D";
      $bcs_file_from = $DATA_ICBC_DIR."/".$BCS_file.$ii.'.tm00.'.substr($BCS_date,2,8);
      $bcs_file_to   = ${DATA_DIR}."/".$BCS_file.$ii.".tm00.".substr($BCS_date,2,8);
      ...
      if(($ii == 3 || $ii == 0) && $normal == 1 && $time_max > 420)
      {
        ...
        {
          $BCS_file1 = "eta.T".substr($BCS_date_L,8,2)."Z.AWIP3D";
          $bcs_file_from_1_12 = $DATA_ICBC_DIR."/".$BCS_file1."12".'.tm00.'.substr($BCS_date_L,2,8);
          $bcs_file_from_1_15 = $DATA_ICBC_DIR."/".$BCS_file1."15".'.tm00.'.substr($BCS_date_L,2,8);
          if ($ii == 0)
          {
        ...
        {
          $BCS_file1 = "eta.T".substr($BCS_date_L,8,2)."Z.AWIP3D";
          $bcs_file_from_1_24 = $DATA_ICBC_DIR."/".$BCS_file1."24".'.tm00.'.substr($BCS_date_L,2,8);
          $bcs_file_from_1_27 = $DATA_ICBC_DIR."/".$BCS_file1."27".'.tm00.'.substr($BCS_date_L,2,8);
    ...
    if($MODEL eq "MM5")
    {
      print FILEL "\n   starting ETA regrid.csh at ", &ctime(time);
      system("$CSH_ARCHIVE/Forecast/RT_A_regrid_en.csh $BCS_date $this_cycle $RAP_start $RAP_end $normal $NODE");
      print FILEL "\n   ending ETA regrid.csh at ", &ctime(time);
...
############## END REGRID - 1st Attempt ########################################
  ...
  if ( $bcs_size < $BCS_SIZE_C)
  {
    ...
    for ($i = 12; $i <= 48; $i+=$step_size)
    {
      ...
      $BCS_file = "eta.T".substr($BCS_date_L,8,2)."Z.AWIP3D";
      $bcs_file_from = $DATA_ICBC_DIR."/".$BCS_file.$ii.".tm00.".substr($BCS_date_L,2,8);
      $bcs_file_to   = ${DATA_DIR}."/".$BCS_file.$ii.".tm00.".substr($BCS_date_L,2,8);
    ...
    {
      print FILEL "\n   starting ETA regrid.csh at ", &ctime(time);
      system("$CSH_ARCHIVE/Forecast/RT_A_regrid_en.csh $BCS_date_L $this_cycle $RAP_start $RAP_end $normal $NODE");
      print FILEL "\n   ending ETA regrid.csh at ", &ctime(time);
