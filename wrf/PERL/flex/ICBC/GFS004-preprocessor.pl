...
sub processData
{
  ...
  # OLD: %BCSDT=("ETA",3,"AVN",6,"AVNFTP",3,"GFS004",3);
  # OLD: $BCSDT{$BCS} = 3;
  ...
  if($BCS_CHANGE == 1 || ($BCS_CHANGE != 1 && $BCS_date == $BCS_date_L))
  {
    ...
    for ($i = 0; $i <= 72; $i+=$step_size)
    {
      ...
      #We need: yyyymmdd_i00_f0
      #   or    yyyymmdd_i12_f0
      $BCS_file = substr($BCS_date,0,8)."_i".substr($BCS_date,8,2)."_f0";
      $bcs_file_from = $DATA_ICBC_DIR."/".$BCS_file.$ii.'_GFS004.grb';
      $bcs_file_to   = ${DATA_DIR}."/".$BCS_file.$ii.'_GFS004.grb';
      ...
      if(($ii == 3 || $ii == 0) && $normal == 1 && $time_max > 420)
      {
        ...
        {
          $BCS_file1 = substr($BCS_date_L,0,8)."_i".substr($BCS_date_L,8,2)."_f0";
          $bcs_file_from_1_12 = $DATA_ICBC_DIR."/".$BCS_file1."12".'_GFS004.grb';
          $bcs_file_from_1_15 = $DATA_ICBC_DIR."/".$BCS_file1."15".'_GFS004.grb';
          if ( $ii == 0)
          {
        ...
        {
          $BCS_file1 = substr($BCS_date_L,0,8)."_i".substr($BCS_date_L,8,2)."_f0";
          $bcs_file_from_1_24 = $DATA_ICBC_DIR."/".$BCS_file1."24".'_GFS004.grb';
          $bcs_file_from_1_27 = $DATA_ICBC_DIR."/".$BCS_file1."27".'_GFS004.grb';
          if ($ii == 0)
          {
    ...
    if ($MODEL eq "MM5")
    {
      print FILEL "\n   starting GFS004 regrid.csh at ", &ctime(time);
      system("$CSH_ARCHIVE/Forecast/RT_A_regrid_gfs004.csh $BCS_date $this_cycle $RAP_start $RAP_end $normal");
...
############## END REGRID - 1st Attempt ########################################
  ...
  if ( $bcs_size < $BCS_SIZE_C)
  {
    ...
    for ($i = 12; $i <= 48; $i+=$step_size)
    {
      ...
      $BCS_file = substr($BCS_date_L,0,8)."_i".substr($BCS_date_L,8,2)."_f0";
      $bcs_file_from = $DATA_ICBC_DIR."/".$BCS_file.$ii."_GFS004.grb";
      $bcs_file_to   = ${DATA_DIR}."/".$BCS_file.$ii."_GFS004.grb";
    ...
    {
      print FILEL "\n   starting GFS004 regrid.csh at ", &ctime(time);
      system("$CSH_ARCHIVE/Forecast/RT_A_regrid_gfs004.csh $BCS_date_L $this_cycle $RAP_start $RAP_end $normal");
      print FILEL "\n   ending  GFS004 regrid.csh at ", &ctime(time);
