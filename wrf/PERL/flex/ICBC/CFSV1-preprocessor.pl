...
#------------------------------------------------------------------------------#
# Bring the NNRP2 input files to the local work dir

      print FILEL "   Starting $BCS data collection at ", &ctime(time);

      $systat = system("$CSH_ARCHIVE/Forecast/RT_get_cfsv1.csh $this_cycle $RAP_start $RAP_end $BCSDT{$BCS} $DATADIR");

      if ($systat != 0)
      {
	  print ("Unable to find CFSV1 data for cycle $this_cycle, stop here!\n");
	  exit (1)
	  }
...
#------------------------------------------------------------------------------#
# Regridding of CFSV1 data onto WRF horizontal grid
