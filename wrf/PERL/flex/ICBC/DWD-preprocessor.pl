...
sub processData
{
  ...
  if($BCS_CHANGE == 1 || ($BCS_CHANGE != 1 && $BCS_date == $BCS_date_L))
  {
    ...
    {
# Get both DWD data files and GFS data files (need soil fields from GFS)
      $ii = sprintf("%02d", $i);
      $BCS_file = &constructDWDfilename( $BCS_date, $ii);
      $bcs_file_from = $DATA_ICBC_DIR."/".$BCS_file; #$DATA_AVN_DIR
      $BCS_file2 = &constructDWDfilename2( $BCS_date, $ii);
...
############## END REGRID - 1st Attempt ########################################
  ...
  if ( $bcs_size < $BCS_SIZE_C)
  {
    ...
    for ($i = 12; $i <= 96; $i+=$step_size)
    {
      ...
      $BCS_file = &constructDWDfilename( $BCS_date_L, $ii);
