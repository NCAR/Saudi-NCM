...
sub processData
{
  ...
  if($BCS_CHANGE == 1 || ($BCS_CHANGE != 1 && $BCS_date == $BCS_date_L))
  {
    ...
    for ($i = 0; $i <= 66; $i+=$step_size)
    {
      $ii = sprintf("%02d", $i);

      # if $BCS == ETA (above) or AVN (below)
      foreach $tile ('037','038','039','040','041','042','043','044')
      {
        $AVNFLAG="096";
        $AVNFLAG="AVN" if ($ii == 0);
        $BCS_file = substr($BCS_date,0,8)."_".substr($BCS_date,8,2)."00_$AVNFLAG"."_";
        $bcs_file_from = $DATA_ICBC_DIR."/".$BCS_file.$tile."_".$ii.'.grib'; #$DATA_AVN_DIR
        $bcs_file_to   = ${DATA_DIR}."/".$BCS_file.$tile."_".$ii.'.grib';

        # Exceptions in order to correct fcst error in 05Z and 17Z cylce
        if(($ii == 3 || $ii == 0) && $normal == 1 && $time_max > 420)
        {
          if($BCS_date == &hh_advan_date($BCS_date_L,12))
          {
            $BCS_file = substr($BCS_date_L,0,8)."_".substr($BCS_date_L,8,2)."00_096"."_";
            $bcs_file_from_1_12 = $DATA_ICBC_DIR."/".$BCS_file.$tile."_12".'.grib'; #$DATA_AVN_DIR
            $bcs_file_from_1_15 = $DATA_ICBC_DIR."/".$BCS_file.$tile."_15".'.grib'; #$DATA_AVN_DIR
            ...
            {
              # this will not be triggered since avn is 6-hourly
          ...
          {
            $BCS_file = substr($BCS_date_L,0,8)."_".substr($BCS_date_L,8,2)."00_096"."_";
            $bcs_file_from_1_24 = $DATA_ICBC_DIR."/".$BCS_file.$tile."_24".'.grib'; #$DATA_AVN_DIR
            $bcs_file_from_1_27 = $DATA_ICBC_DIR."/".$BCS_file.$tile."_27".'.grib'; #$DATA_AVN_DIR
            if ($ii == 0)
            {
    ...
    if($MODEL eq "MM5")
    {
      print FILEL "\n   starting AVN regrid.csh at ", &ctime(time);
      system("$CSH_ARCHIVE/Forecast/RT_A_regrid_avn.csh $BCS_date $this_cycle $RAP_start $RAP_end");
      print FILEL "   ending  AVN regrid.csh at ", &ctime(time);
...
############## END REGRID - 1st Attempt ########################################
  ...
  if ( $bcs_size < $BCS_SIZE_C)
  {
    ...
    {
      print FILEL "\n   starting AVN  regrid.csh at ", &ctime(time);
      system("$CSH_ARCHIVE/Forecast/RT_A_regrid_avn.csh $BCS_date_L $this_cycle $RAP_start $BCS_end $normal");
      print FILEL "   ending  AVN regrid.csh at ", &ctime(time);
