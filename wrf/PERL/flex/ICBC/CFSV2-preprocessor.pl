...
#------------------------------------------------------------------------------#
# Bring the CFSV2 input files to the local work dir

      print FILEL "   Starting $BCS data collection at ", &ctime(time);

      $systat = system("$CSH_ARCHIVE/Forecast/RT_get_cfsv2.csh $this_cycle $RAP_start $RAP_end $BCSDT{$BCS} $DATADIR");

      if ($systat != 0)
      {
	  print ("Unable to find CFSV2 data for cycle $this_cycle, stop here!\n");
	  exit (1)
	  }
      print FILEL "   Finish $BCS data collection at ", &ctime(time);

#------------------------------------------------------------------------------#
# Regridding of NNRP2 data onto MM5 horizontal grid

  if($MODEL eq "MM5")
  {
      print FILEL "\n   starting $BCS regrid.csh at ", &ctime(time);
      system("$CSH_ARCHIVE/Forecast/RT_A_regrid_nnrp2.csh $BCS_date $this_cycle $RAP_start $RAP_end $normal $NODE $InterpType");
      print FILEL "   ending $BCS regrid.csh at ", &ctime(time);

#------------------------------------------------------------------------------#
# Regridding of CFSV2 data onto WRF horizontal grid
# the argument $BCS controls the forcing data within RT_A_wrf_wps.csh 
