...
sub processData
{
  ...
  if($BCS_CHANGE == 1 || ($BCS_CHANGE != 1 && $BCS_date == $BCS_date_L))
  {
    ...
    {
# Get both GEM data files and GFS data files (need soil fields from GFS)
      $ii = sprintf("%02d", $i);
      $BCS_file = &constructGEMfilename( $BCS_date, $ii);
...
############## END REGRID - 1st Attempt ########################################
  ...
  if ( $bcs_size < $BCS_SIZE_C)
  {
    ...
    for ($i = 12; $i <= 96; $i+=$step_size)
    {
      ...
      $BCS_file = &constructGEMfilename( $BCS_date_L, $ii);
