...
foreach $domain (1..$NUM_DOMS) {
  ...
  ### Add domain ID to error dirstribution stats ASCII files

  foreach $wrfFreq (<*_freq_*_??hr_wrf.txt>) {
     $wrfFreqNew = $wrfFreq;
     $wrfFreqNew =~ s/wrf/wrf_d${domain}/; 
  ...
  foreach $refFreq (<*_freq_*_??hr_ref.txt>) {
     $refFreqNew = $refFreq;
     $refFreqNew =~ s/ref/ref_d${domain}/; 
...
sub sfc_plot {
  ...
  open(REF,"sfc_stats_ref_d${domain}.txt") if (-e "sfc_stats_ref_d${domain}.txt");
...
sub upr_plot {
  ...
  foreach $lead (@leads) {
     ...
     next if (! -e "upr_stats_wrf_d${domain}_${lead}hrs.txt" ||
              ! -e "upr_stats_ref_d${domain}_${lead}hrs.txt");

     open(WRF,"upr_stats_wrf_d${domain}_${lead}hrs.txt");
     open(REF,"upr_stats_ref_d${domain}_${lead}hrs.txt");
...
sub histogram {
  ...
  foreach $freq (<*freq*wrf_d${domain}.txt>) {

     next if($freq =~ /psfc/);

     $freq =~ /(\w+)_freq_(\w+)_(\d+)hr_(\w+)_d(\d+)\.txt/;
