...
def create_parser():
    ...
    parser.add_option('-a', '--all_sams',  '--all-sams', dest='all_sams',
            default=False, action='store_true', help=" Import all SAMS NetCDFs")
...
class import_sams_to_db(object):
    ...
    def do_import(self):
        ...
        for range_name in ranges:
            ...
            if self.options.all_sams:
                sams_files = []
                for (dirpath, dirnames, filenames) in os.walk(sams_dir):
                    for sams_file in sorted(filenames):
                        sams_files.append(os.path.join(sams_dir, sams_file))
                    break   # do not recursive
                for sams_file in sams_files:
                    python_cmd = '{s} -r {r} -i {i}'.format(s=python_script,
                            r=range_name, i=sams_file)
                    if self.options.debug: 
                        python_cmd = '{c} --debug'.format(c=python_cmd)
                    run_shell_command(python_cmd,print_commands=False)
                        
            elif os.path.exists(input_sams):
