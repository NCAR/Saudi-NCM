...
#         #    self.set_range(range_name)
    ...
    def compute_score(self, value, prev_value, time_diff, climatic_data, threshold,
        ...
        method_name = '{m}.{n}()'.format(m=MODULE_NAME, n='compute_score')
        ...
        if value is not None and value != missing_value:
            ...
            #if prev_value is not numpy.ma.masked:
            if do_temporal_score and prev_value is None or prev_value == missing_value \
                    or numpy.ma.is_masked(prev_value):
            ...
            if do_temporal_score:
                ...
                if math.fabs(time_diff) > (sams_qc.TEMPORAL_TIME_DIFF_MAX * 4):
                    ...
                    if threshold is not None:
                        ...
                        self.error_message('The temporal threshold is required but does not exist!')
            #else:
    ...
    def get_time_interval(self, time_offset_list):
        method_name = '{m}.{n}()'.format(m=MODULE_NAME, n='get_time_interval')
        debug = False
        #debug = not debug
        count_01min = 0
        count_05min = 0
        ...
        if debug:
            ...
            self.debug_message('{m} {d}'.format(m=method_name, d=debug_msg), debug)
        return time_interval

    def set_range(self, range_name):
        if range_name is None or range_name == "" :
            self.error_message('%s range_name is missing' % (method_name))
            sys.exit(-2)
    ...
    def setup_qc_data_climatic(self, range_id, climatic_month):
        ...
        for var_name, climate in zip(sams_qc.VAR_LIST, climate_list):
            if climate is None:
                self.error_message('check climatic min/max for {v}'.format(v=var_name))
    ...
    def setup_qc_data_temporal(self, range_id, time_interval):
        ...
        for var_name, temopral in zip(sams_qc.VAR_LIST, temopral_list):
            if temopral is None:
                self.error_message('Check temporal difference for {v}, time interval={t} index={i}'.format(
...
class sams_qc_tools(object):
    ...
    def get_temporal_threshold_map(db_actor, interval_in_seconds):
        ...
        if   interval_in_seconds >= 300:
            ...
            if debug:   print('called get_15min_map {t}'.format(t=threshold_map))
        elif interval_in_seconds > 60:
            threshold_map = get_5min_map(db_actor)
            if debug:   print('called get_5min_map {t}'.format(t=threshold_map))
            if threshold_map is None:   threshold_map = get_15min_map(db_actor)
        else:
            threshold_map = get_1min_map(db_actor)
            if debug:   print('called get_1min_map {t}'.format(t=threshold_map))
            if threshold_map is None:
                threshold_map = get_5min_map(db_actor)
                if debug:   print('called get_5min_map {t}'.format(t=threshold_map))
                if threshold_map is None:   threshold_map = get_15min_map(db_actor)
        if threshold_map is None:
            error_message(' %s can not find threshold for %d minutes temporal difference' % (method_name, interval_in_seconds/60))
