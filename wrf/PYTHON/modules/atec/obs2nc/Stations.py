...
class Stations(object):
    ...
    SQL_SELECT_TEMPLATE = 'SELECT DISTINCT S.%s, S.%s, latitude, longitude, %s FROM %s %s'
    ...
    def add_stations(self, station_table_name, id_pattern=None, station_ids=None, 
        ...
        debug = False
        debug = debug or l_debug or os.environ.get("OPT_DEBUG") is not None
        debug_sql = False
    ...
    def add_stations_with_real_data(self, station_table_name, id_pattern=None,
        ...
        sql_where_and_more = "{w} {o}".format(w=sql_where, o=sql_order_by)
        sql_select = Stations.SQL_SELECT_TEMPLATE % (common_col_name,
                unique_col_name, elevation_column_name,
    ...
    def add_stations_by_sql(self, sql_select, id_pattern=None,
        ...
        added_stations = 0
        if debug_sql:   print ('{m} sql_select: {s}'.format(m=method_name, s=sql_select))
        try:
        ...
        if debug:   print('{m} added_stations: {c}'.format(m=method_name, c=added_stations))
    ...
    def get_station_index(self, range_code):
        ...
        if self.station_list:
            for index, station in enumerate(self.station_list):
                if debug: print ('%s range_code: %s, unique_site_id: %s'
                        % (method_name, station.get_range_code(), station.get_unique_site_id()))
                #if range_code == station.get_unique_site_id() \
                #        or station.get_range_code_org() == range_code \
                #        or range_code.startswith(station.get_range_code()):
                if station.get_unique_site_id() == range_code \
                        or station.get_range_code_org() == range_code:
                    station_index = index
                    break
                index = index + 1
    ...
    def get_station_index_by_unique_site_id(self, unique_site_id):
        debug = False
        #debug = debug or os.environ.get("OPT_DEBUG" is not None) is not None
        #debug_sql = False
        #debug_sql = debug_sql or os.environ.get("OPT_DEBUG_SQL") is not None
        #method_name = '     %s.%s()' % (MODULE_NAME, 'get_station_index')
        method_name = '     get_station_index()'
