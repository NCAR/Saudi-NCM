...
class cfsdata():
    ...
    def remotegdas(self,
        ...
        cdate = sdate -timedelta(hours=6)   # ensure complete sfc data (6-hours forecast)
        while cdate <= edate:
            ...
            for atmfld, fhead, dhour in zip(atmflds, self.filehd, self.datahh):
                ...
                hsicmd = 'hsi "ls %s"'%cfsfile
                print 'inside remotegdas: %s '%hsicmd
                ...
                if re.search(r'No such file or directory', out):
                   ...
                   return 'F', cdate
                hsicmd = 'hsi "cd %s; rm -f my*tar; '%self.tmpdir + 'ln -s %s %s"'%(cfsfile, self.tmpfile)
...
# extract needed files only 
                   hcmd = 'htar -L%s -xvf %s' %(self.wtedf,lfile)
                   out,err = runcmd(hcmd)
              #    print err, hcmd 
                   for fold, fnew in zip(wantednams, wantednews):
                     # print fold, fnew 
                       ...
                       print '%s -> %s' %(fold, fnew)
                   out,err = runcmd('rm -f %s'%self.wtedf)
                else:
                   out,err = runcmd('hsi "rm lfile"')
    ...
    def remotecdas(self,
        ...
        hours   = int(sdate.strftime('%H'))
        fstdate = sdate - timedelta(hours=hours)
        cdate = sdate - timedelta(hours=6)
        hours   = int(cdate.strftime('%H'))
        cdate = cdate - timedelta(hours=hours)
        hours   = 24 - int(edate.strftime('%H'))
        lstdate = edate + timedelta(hours=hours)
        ...
        while cdate < lstdate:
            ...
            for atmfld,fldkey,fhead in zip(atmflds,fldkeys,self.filehd):
                print 'inside remotecdas: %s %s'%(yy,cymd)
...
#*** ls: No such file or directory [-2: HPSS_ENOENT]
                   if re.search(r'No such file or directory', out):
                      ...
                      if re.search(r'No such file or directory', out):
                         ...
                         udir = os.path.join(self.udir,cdate.strftime('%Y'),cdate.strftime('%m'))
                         createdir(udir)
                         fetchcfsforecast(initime, cdate, edate+timedelta(days=1),udir, self.mem)
                         return 'P', cdate
...
# extract needed files only 
                      ...
                      frname = filerename(wantednams, fhead, fstdate, lstdate, cdate, self.udir)
                      (out,err) = runcmd('rm -f %s'%self.wtedf)
                   else:
                      (out,err) = runcmd('hsi "rm lfile"')
...
def filerename(fileset, fldkey, fstdate,lstdate, cdate, ldir):
...
# cdas1.t18z.sfluxgrbf06.grib2
    ...
    for file in fileset:
        if os.path.isfile(file):     # cfs cdas file name convention:
            ...
            h06   = re.search(r'f(\d+).',file)                # data initial hours (UTC: hh)
            if h06 == None: 
               h06 = '00'
            else:
             # h06   = re.search(r'f(\d+).',file).group(1)       # data initial hours (UTC: hh)
               h06   = h06.group(1)       # data initial hours (UTC: hh)
            
            if itime:
                dt = int(itime) + int(h06)
            else:
            ...
            if sfcutc >= fstdate and sfcutc < lstdate:
               print file, '->', filen 
