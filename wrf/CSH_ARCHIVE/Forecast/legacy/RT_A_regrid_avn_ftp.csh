...
if      ( ${#argv} < 5 )  then
	echo "usage: $0 avn_date cycle_date start_date end_date cold_start [ukmet]"
	...
	echo  $end_date is the needed hour of current AVN fcst"
        echo "cold_start = 1 for cold start"
        echo "umket: for using upper-air ECMWF fields from UKMET."
	echo "where all dates are given in CCYYMMDDHH"
	exit ( 1 )
endif
...
if ($debug > 0) then
 ...
 setenv OUTPUT_DATA_DIR /data/input/avnftp
...
## AVN fields used for UKMET are constant, use only first and last

if ($ukmet == "ukmet" || $ukmet == "UKMET") then

set LIST_FH = ( ${avn_date}_fh.0000 \
                ${avn_date}_fh.0060 )
else

## Use AVN files every 3hr until +60hr
...
if ($ukmet == "ukmet" || $ukmet == "UKMET") then
( time ${EXECUTABLE_ARCHIVE}/regridder_ukmet.exe ) >> print.out_${avn_date}
else
( time ${REGRIDDER_EXE} ) >> print.out_${avn_date}
endif
