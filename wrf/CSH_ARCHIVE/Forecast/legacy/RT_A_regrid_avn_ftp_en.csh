...
echo  " ------------------ RT_A_regrid_avn_ftp_en.csh ------------------------"
###############################################################################
...
set ltrs = ( AA AB AC AD AE AF AG AH AI AJ AK AL AM AN AO AP AQ AR AS AT AU AV AW AX AY AZ \
             BA BB BC BD BE BF BG BG BI BJ BK BL BM)
set icount = 0
...
foreach f ( ${avn_date}_fh.0000 \
	  ...
	  ln -s $DECODING_AVN_DIR/$f GRIBFILE.${ltr}
...
# Get the Vtable:
#
if( $start_date > 2005053112 ) then
 # 4 layer soil
 cp $PREGRID_VTABLE_AVN_ALL Vtable
else
 # 2 layer soil
 cp $PREGRID_VTABLE_AVN_2SL Vtable
endif
