...
if      ( ${#argv} <= 5 )  then
	echo "usage: $0 ukmet_date cycle_date start_date end_date cold_start"
	...
	echo "RT_A_regrid_ukmet.csh $argv[*]"
endif
...
## Look for the avn data in the "data" directory.

pushd $OUTPUT_DATA_DIR

if ( `find . -name ${avn_date}_ukmet.\*.nc -print | wc -l` >= $num_avn ) then
...
if (-f pregrid.namelist) mv pregrid.namelist AVNpregrid.namelist
if (-f Vtable) mv Vtable AVNVtable
...
echo "--------------- pregrid UKMET -----------------------" >> print.out_${avn_date}

( time ${EXECUTABLE_ARCHIVE}/pregrid_ukmetnc.exe ) >>! print.out_${avn_date}
...
echo ""
echo "-- Catenate UKMET upper-air with AVN surface fields --" >>! print.out_${avn_date}
...
## Remove UKMET upper-air fields from AVN files
   ...
   if (-f AVN${f}) then
       ${EXECUTABLE_ARCHIVE}/notinukmet.exe AVN$f >>! print.out_${avn_date}
   else
...
## Catenate files
   ...
   if ((-f ${f}) && (-f AVN${f}.notinukmet)) then
         echo "cat AVN${f}.notinukmet >>! $f" >>! print.out_${avn_date}
