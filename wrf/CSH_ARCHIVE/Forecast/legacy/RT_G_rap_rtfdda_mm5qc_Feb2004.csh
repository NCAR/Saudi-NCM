...
if ( ${#argv} < 4 ) then
        echo ""
        echo "ERROR (usage): $0 start_date end_date this_cycle last_cycle"
...
if ((-l all.obs_mm5qc) || (-e all.obs_mm5qc)) then
     rm all.obs_mm5qc
endif
...
${EXECUTABLE_ARCHIVE}/latlon.exe -i $mm5
   ...
   cp oberr.181.txt  /data/cycles/vandenb/ETA_NEWQ/${this_cycle}/.
   cp oberr.281.txt  /data/cycles/vandenb/ETA_NEWQ/${this_cycle}/.
...
## Cat the QCed hourly data into a single file
           ...
           echo "cat ${ccyymmddhh}.${obs}_mm5qc >>! all.obs_mm5qc"
                 cat ${ccyymmddhh}.${obs}_mm5qc >>! all.obs_mm5qc
       endif
...
$CSH_ARCHIVE/Forecast/RT_all.obs_trim-merge.USA ${obs}_mm5qc 59 latlon.txt >  /dev/null
...
## Rename the files
   ...
   if (-e ${ccyymmddhh}.${obs}_mm5qc) then
#      echo "${ccyymmddhh}.${obs}_mm5qc"

## Reformat the QCed data
       echo "Reformat file ${ccyymmddhh}.${obs}_mm5qc."
       set qc_out = qc_out_${ccyy}-${mm}-${dd}_${hh}:${mn}:${ss}.0000
       cp ${ccyymmddhh}.${obs}_mm5qc $qc_out
       ${EXECUTABLE_ARCHIVE}/RT_fdda_reformat.pl $qc_out >/dev/null
...
#	Clean up
   ...
   if ($save_data > 0) then
   cp all.obs_mm5qc  /data/cycles/vandenb/ETA_NEWQ/${this_cycle}/.
   endif
