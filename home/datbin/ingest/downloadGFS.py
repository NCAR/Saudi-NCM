...
GFS_BASE_URL  = 'https://www.ftp.ncep.noaa.gov/data/nccf/com/gfs/prod/gfs.'
GFS_BASE_URL2 = 'https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.2015011500/'
...
if 0 > offhours:
    for hour_offset in range(0,24):
        print "looking at hour offset: {h}".format(h=hour_offset) 
        ptimeutc = ctimeutc - timedelta(hours = hour_offset)
        urlsite = '{b}{d}/{h}/'.format(b=GFS_BASE_URL, d=ptimeutc.strftime('%Y%m%d'), h=ptimeutc.strftime('%H'))
        try:
...
localdatadir = '{p}/k1206/datainput'.format(
        p=os.environ.get('WORKDIR2','/lustre/project'),
        u=os.environ.get('USER','k1206'))
if not os.path.exists(localdatadir):
    localdatadir = '/lustre/project/k1206/datainput'.format(u=os.environ.get('USER','k1206'))
...
# 66.172.231.102
# 129.19.157.17
print "retrieving from {u}".format(u=urlsite) 
